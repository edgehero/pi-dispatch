...
- **Contract**:
  ...
  - **The same sweep reclaims the session NETWORK, on the directory's clock** (issue #337). After the
    ...
    throws skips the sweep rather than sweeping on half the evidence. **The container guard gates the
    detach**, not the removal: docker refuses to remove a network a live container is on, so the only harm
    available is stripping the proxy off a shell someone is sitting in. It covers a RUNNING session container
    and no more, because `.Containers` lists running endpoints only: the launch window, where a container is
    in `created` state and invisible to this check and to `listRunningSandboxes` alike, is covered by the
    directory rather than by this. **The sweep is injected rather than
...
- **Acceptance**: The sandbox argv contains every member of `ISOLATION_FLAGS` (asserted against the
  ...
  id the pass listed on disk, or whose id is running, is passed over in SILENCE, because a line per retained
