...
- **`run.secrets` (ALL FOUR trigger kinds, optional map of env-var name to opaque reference) and
  ...
  **`secret-name-reserved` reserves every variable pi reads for the job's provider, set on this host or
  ...
  environment, which is the DEFAULT, and its `undefined` arrived as `?? []` so the empty set was silent:
  ...
  also reserves the provider's OAuth variable, which no ordinary host sets and a presence filter therefore
  never held, and which pi reads BEFORE the API key. The set is bounded to the job's OWN provider: an
