...
- **The venue stamp** (issue #277): one backend name beside the transcript, naming the venue whose container
  ...
  **The resolve path stages the transcript from ONE DESCRIPTOR, and re-checks the name after it** (issues
  ...
  re-check could only shrink (measured against a live second process: 58 of 117 resumes staged an attacker's
  transcript before, 0 of 16,594 after). The by-path re-check answers a swap that lands BEFORE the open,
...
- **Every read in a key directory is an `lstat` first, regular files only, and every sidecar write goes
  ...
  **The key DIRECTORY is checked too, and that was the one shape the lstat-per-name rule could not see**
  ...
  on the first draft: 195 of 757 successful resumes staged the attacker's transcript and reported `resumed`).
  ...
  **The WRITE path cannot be closed the same way**, because there is no descriptor for a directory Node can
  ...
  removes the check-to-lock window and makes a later swap REPORTABLE, but a swap after the last check still
  takes the transcript and the sidecars to wherever the name points. That case now reports
  `key-not-a-directory` instead of `promoted: true`, which is the honest half of it.
...
- **Acceptance**: Given two jobs whose keys differ only in repository or only in branch, their `/session`
  ...
  resumes the transcript it judged and no byte of the replacement reaches the container; given one swapped
  after the write path's last check, the promotion reports `key-not-a-directory` rather than `promoted`;
...
- **Location**: `PI_SCOPED_LIMITS_FILE` (unset = no scoped caps and no scoped concurrency; the worker loads
  ...
  fail-closed default does not move. The one-job-per-folder mutex for local jobs is CODE, not configuration: it holds with no file, an
