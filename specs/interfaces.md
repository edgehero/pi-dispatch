...
- **Contract**:
  ...
  - **What the daemon is observed to provide (issue #345).** The `docker info --format={{json .}}` read the job user
    ...
    `mounts`, `devices` or `hooks_dir` key (in any TOML spelling; an escaped key counts as one) appears in
