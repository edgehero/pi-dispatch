...
- **Location**: `PI_PAUSE_WINDOWS_FILE` (unset = feature off; the worker loads `[]`). Since issue #357
  ...
  in this project to read a `.env` file's contents and is bounded to deciding what a message says
...
- **Location**: `PI_SCOPED_LIMITS_FILE` (unset = no scoped caps and no scoped concurrency; the worker loads
  ...
  when that key has no value, and `doctor` reads the deployment's `.env` for this key alone to soften its
  warning rather than cry wolf at a converged deployment: the same change, the same bounds and the same
