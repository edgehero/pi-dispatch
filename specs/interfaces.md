...
- **Contract**:
  ...
  - **The user-namespace field (issue #354, the seam for a native Podman backend).** The spec carries `userns`,
    ...
    rootless Podman 5.8.1). No venue builds with it yet; the job path's runtime binary is a seam (`bin`, `docker` by
    default) in `run-container`, the image and egress preflights, the network helpers, the stop and the reaper.
...
- **Acceptance**: The probe argv contains every member of `ISOLATION_FLAGS` (asserted against the imported
  ...
  `/sys` mask it holds; given a mountinfo read that failed, mountSet is not read back (issue #345).
