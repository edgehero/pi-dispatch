...
- **Contract**:
  ...
  - **The same module answers `doctor`'s different question, and it is a different question** (issue
    #286). The container path only ever needs the keys the host HOLDS, so `findEnvKeys(provider, hostEnv)`
    is the whole answer there. `doctor` has to NAME a variable that is *not* set, which that call cannot
  ...
  - **On the env path the NAMES come from pi and the VALUES decide, and the two are not the same question**
    (issue #311). `findEnvKeys`' presence test falls back to the real `process.env`, deliberately, because
    `doctor` needs it. The credential resolver therefore keeps only the names the env it was HANDED actually
    carries, and takes the `auth.json` fallback when none remain. Without that filter a host that merely
    exports a provider variable makes the resolver answer `{ NAME: undefined }`: a credential-shaped result
    with no credential in it, no fall through to the login that would have worked, and a container that
    reaches the provider unauthenticated with the budget already spent.
...
- **Evidence (upstream)**: `earendil-works/pi @ 5e336cf → packages/ai/src/env-api-keys.ts:69-71 → getApiKeyEnvVars`
  ...
  `→ dist/env-api-keys.js → findEnvKeys` — returns only the names PRESENT in the env it is handed, and
