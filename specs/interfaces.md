...
- **Status**: the grammar, the profile table, the config keys, the `after` hold and the pre-spend refusals
  are LANDED. **What is still unbuilt is the running of a check**: Invocation, the Verdicts table and the
  bounds that govern polling describe the enforcement slice, and until it lands a `profile` condition is
  refused pre-spend as `wait-profile-unknown` rather than waited on — which is the fail-closed direction,
  and is why `Location`'s parenthetical is now true rather than aspirational. Deliberately not an
...
- **Producer/Consumer**: operator → worker, through the environment. The receiver carries `run.waitFor`
  into job data and evaluates nothing. The admin extension declares no profile and, as of this slice, reads
  no held state either -- the `wait:` keyspace has a writer and no reader until the panel slice lands.
- **Location**: `PI_WAIT_PROFILES` (unset = the feature is off). Note what the Status bullet above means
  for this one: until the checker lands, EVERY `profile` condition refuses pre-spend whether or not the
  variable declares it, because nothing can run a check yet. The table is parsed and validated at boot; it
  is not yet consulted by any refusal. `name:/absolute/path` pairs, comma separated, each entry split on its FIRST colon so a
