...
(`cron.mjs -> reconcileGated`), the receiver's router (`route.mjs`), `doctor`, and the admin console's
fleet views (`admin/src/dashboard.ts`, `admin/src/read-model.mjs`), the last three through the shared
`readLiveHosts`. This list said "the cron gate, and nothing else" until issue #302, and had been false
since the console started reading the keyspace; it is corrected here rather than left, because the entry's
own reasoning leans on it -- the index leak below is "handled by the READER", and the reader is now a
panel an operator looks at, not only a gate that abstains.
