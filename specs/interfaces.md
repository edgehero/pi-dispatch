...
- **Contract**:
  ...
  - **One credential fact is written down here rather than derived, and only one.** Whether a variable is
    ...
    them expires. `doctor` holds a suffix rule (`/_OAUTH_TOKEN$/`) rather than a name set, because the
    expensive direction is the false green — a set would silently bless the next provider's OAuth
    variable — and the rule is pinned against pi in both directions rather than against a second copy.
