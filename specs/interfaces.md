...
- **Contract**:
  ...
  - **What the daemon is observed to provide (issue #345).** The `docker info --format={{json .}}` read the job user
    ...
    on Docker, and on Podman only when `/etc/containers/mounts.conf` exists and is empty, no `volumes` or `mounts` key
    appears in `/usr/share/containers/containers.conf`, `/etc/containers/containers.conf` or a `*.conf` in the four
    drop-in directories, every one of those that exists is readable, `/proc/sys/crypto/fips_enabled` is not `1`, and
    the service's socket is a unix path on this host. Both are reported in `worker_started`, logged as
  ...
  - **A container that outlives its `docker run` (issue #345).** Every job argv carries `--cidfile=<job dir>.cid`,
    ...
    running), so it is stopped and removed by ID and the job fails as INFRA `container-detached`: retried, and NOT
    refunded, because a container started. A plain name conflict writes no cidfile and is never touched.
...
- **Contract**:
  ...
  - **The reads**, each a verdict `{ property, ok, warn?, detail }`, where `warn` means NOT READ BACK and is never
    ...
    - `mountSet`: `docker inspect .Mounts` keyed by destination and read-write flag against the spec's own
      ...
      segment, measured with the builder's argv on Docker Desktop, rootful Docker and rootful Podman. A mountinfo
      read that failed or held no mount line is not read back; a mount point is printed only in printable bytes.
...
| 2026-09-15 | Issue #345, runtime observations. **`INT-CONTAINER-RUNTIME-CONTRACT` AMENDED**: two bullets. "What the daemon is observed to provide": `daemonAppliesBounds` gates `isolation` and `runtimeAddsNoMounts` gates `mountSet`, from the same cached `docker info` read (now made on every platform) and this host's Podman files, refused only under a floor, at boot (unanswered exits 1, answered exits 2) and per job as `backend-floor-unobserved`. "A container that outlives its `docker run`": the job argv's `--cidfile`, and a never-started exit whose container this attempt created and that is still there is stopped and removed by ID and fails as INFRA `container-detached`, retried and not refunded; `created` stays never-started. **`INT-RUN-HISTORY-FILE-CONTRACT` AMENDED**: the `reason` enum gains `container-detached`. **`INT-LIVE-PROBE-CONTRACT` AMENDED**: `mountSet` also reads the reading container's `/proc/self/mountinfo` by a constant `cat` against an exact list plus three kernel trees, failing `runtime-mount`. **`INT-SANDBOX-CONTRACT` UNCHANGED, checked**: the sandbox argv carries no cidfile and no new observation gates it. |
