...
- **Contract, container → worker**:
  ...
  **Named residuals of the provider-refusal row (issue #437), none of which the runner can see:**
  ...
  - A genuine refusal whose body happens to carry one of pi-ai's retry words (a `500` inside a request id,
    ...
    cheap direction, on purpose.
  - The exit line's `message` is capped at 2000 characters with a `... [truncated N chars]` marker, on
    both exit-line paths, so an HTML error page cannot push the line's `code` and `reason` out of the
    worker's 8 KiB tail.
