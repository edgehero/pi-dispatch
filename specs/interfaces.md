...
- **`run.waitFor` (webhook kinds only, optional array of 1..4 one-key condition objects)**: the conditions
  ...
  **A DUPLICATE key refuses the whole file, anywhere in it** (issue #313), and the entry above is the
  ...
  The refusal also guards the two RAW reads in `triggers-file.mjs` that the validator never sees. The
  disarm path is the sharper one: it parses, collapses the duplicate to its last value and then rewrites
  the whole file from the parsed array, so recording one disarm would delete the shadowed key from disk
  permanently -- the destruction that path's own comment forbids, arriving through the door the comment
