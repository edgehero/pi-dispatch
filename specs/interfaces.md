...
- **Contract**:
  ...
  **resolved** name, `run.backend` else the deployment default (`PI_BACKENDS[0]`), computed by
  ...
  processor catches that refusal rather than letting it skip the record, so the job is refused and recorded
  instead of retried as an infrastructure failure. ABSENT means the key is absent: an explicit `null` is a
