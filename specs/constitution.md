...
- **Why**: pi ships no permission system. Untrusted issue text drives an unrestricted agent holding our
  ...
  **A THIRD CONTAINER SHAPE, the live probe (issues #278 and #344), and it meets all four of the carve-out's
  ...
  peer and connection scripts), given nothing but a nonce, a run number, a port, a lifetime in seconds, the
  configured proxy's name and port, and the second peer's own name plus the names and addresses its daemon
  reports. **The
  peers are attached to a network, so that is argued on its own rather than through the mounts**: a peer's
  `--internal` network has at most two members, the peer and the operator's own egress proxy, which is the network
  a job gets under `CONST-EGRESS-POLICY-IN-THE-ARGV`, so a peer can open no connection a job could not; the
  listener answers every connection with the nonce and reads nothing it is sent, and the connection script
  reduces whatever comes back to one word per target, so no byte from beyond the fixture reaches a verdict or the
  operator's terminal. Each is removed when its phase ends, by the ID `docker run -d` printed (a container
