...
- **Why**: pi ships no permission system. Untrusted issue text drives an unrestricted agent holding our
  ...
  **A THIRD CONTAINER SHAPE, the live probe (issues #278 and #344), and it meets all four of the carve-out's
  ...
  configured proxy's name and port, and the names and addresses the daemon reports for the second peer. **The
  peers are attached to a network, so that is argued on its own rather than through the mounts**: a peer's
  `--internal` network has exactly two members, the peer and the operator's own egress proxy, which is the network
