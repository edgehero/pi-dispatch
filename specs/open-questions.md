...
- **Update (2026-08-25, issue #202) -- the first of those two findings is REFUTED, and the correction
  ...
  refuses pre-spend. **Update (2026-09-25, issue #427):** the refutation measured the SDK and `fetch` on their
  own, never after pi was loaded, and in the runner pi IS loaded: the pinned pi's npm `undici` 8.5.0 replaces the
  flag's dispatcher on load, so the provider call went direct after all (measured in the job image: `401`
  through the proxy before the import, `ENOTFOUND` after it). The original observation was therefore right
  about the outcome, and its cause was a third one, which neither this row nor #202 named. Whether the pin of
  that day already did this was not re-measured. The design conclusion stands (a proxy carries the provider
  call), because the runner now restores the proxy itself. `docs/sandbox.md`'s egress section is replaced by `docs/egress.md`, which carries the
...
- **Action on bump**: re-verify `dist/core/sdk.d.ts` **in the new tarball** — not on `main` — before
  ...
  whether `authStorage`/`modelRegistry` are gone or merely deprecated. Since issue #291 the option set
  carries a second load-bearing surface: `excludeTools` is passed by the runner and validated against
  the built-in tool set in three places, so also re-verify in the new tarball that the option survives
  with its `string[]` type, that `core/tools`' `allToolNames` is still the seven the hand-written
  `EXCLUDABLE_TOOL_NAMES` restates, and that pi still IGNORES unknown exclusion names silently (that
  fact is why the validation exists; if a bump makes pi throw on them, the belt-and-braces split is
  re-argued, not merely re-pinned). The bolts that fire: `image/runner/test/pinned-api.test.mjs`'s
  sdk.d.ts pins and `worker/test/exclude-tools.pinned.test.mjs`.
...
- **Resolution**: one daily `.unref()`'d interval, armed at the end of boot, running the three reaper
  ...
  boot and every tick, and a fault is one log line rather than a crash. (Both amendments below narrow
  those three to FAULTS and give each sweep's per-entry verdicts their own names.) The timer is registered in
...
- **AMENDED (issue #375, 2026-09-22), the same distinction on the session store.** The session reaper now
  `lstat`s each entry before it judges anything, so an entry that is not a real directory is left where it
  stands rather than resolved through, and its per-entry outcome is `session_not_reaped {key, reason}` on the
  `sandbox_network_not_reaped` pattern. `session_reaper_skipped` keeps only the fault. Worth stating because
  a shape MOVED between the two: a stray file in the store used to reach the per-entry catch as an ENOTDIR
...
- **Status**: **ACCEPTED RISK -- RATIFIED 2026-09-09** (issue #281). The ratifying act is
  `requirements.md`'s Scope, which now names Azure DevOps as serviced WITH the one property it cannot
  have -- a scope statement that names the forge is the explicit acceptance this row wanted, made where
  the project states what it services rather than in a register a first-time reader never opens.
- **Why ratified rather than decided down, which was the live alternative this row itself offered**:
  features kept landing on Azure while the row kept asking (#187 widened `run.replicas` onto it), which
  is drift toward ratification by accretion -- the wrong way to accept a risk. Deciding down lost on the
  record: the arm ships end to end (receiver verification, worker auth, prompt builders, its own job
  image), the risk is bounded by the terms below, and deleting a shipped, bounded, documented arm to
  avoid saying "we accept this" would answer a paperwork debt with real deletions nobody asked for. The
  bounds already listed are the TERMS of the ratification, not background.
...
- **What would REOPEN it** (the `OQ-014` precedent: a ratified row's `Needs` is replaced by its reopen
  conditions): Microsoft shipping an HMAC or signed-timestamp option on Service Hooks, which turns this
  from an unclosable risk into a work item; the author gate weakening on Azure, since it is the bound
  that keeps a secret-holder from naming themselves an owner; or dedup moving off the body-carried `id`
  in a way that widens the replay surface. A NEW forge arriving does not reopen it: the Scope pin test
  (`worker/test/forges.test.mjs`) refuses a forge the sentence does not name, which is this row's
  acceptance discipline generalized.
...
- **Status**: **CLOSED** — the receiver routes them (issue #57). Kept for the argument, which the design still rests on.
...
- **What closed it**: the receiver reads the host registry at enqueue and picks a queue, exactly as this
  row predicted. Each worker publishes the NAMES of the secret and wait profiles it declares (`caps` on its
  registry row, names only, never the resolver paths behind them), and `capabilities.mjs` owns the rule so
  the worker's refusal and the receiver's routing cannot drift apart.
  The rule ABSTAINS in four cases and every one lands on the previous behaviour, which is what makes it
  safe in front of every delivery: the job binds nothing; every live host can serve it, where the shared
  queue is strictly better because it load-balances; no host can serve it, where the existing pre-spend
  refusal is the honest answer and already names what to fix; or no capable host has a queue of its own and
  the registry could not be read. The receiver caches the read for five seconds with a single in-flight
  request, so a delivery burst costs one read rather than one per delivery, and it caches the FAILURE too.
  A host must be BEATING to attract routed work rather than merely unexpired: the 90s TTL is a crash
  backstop deliberately set at six missed beats, while a job routed onto a stopped host's queue waits for
  that host to return, so routing uses three beats instead.
...
## OQ-033 — The scheduler stall counter is one counter for the whole fleet

- **Status**: **CLOSED** — not by namespacing the counter, which would have been wrong, but by fixing the two defects underneath it (issue #267). Kept for the corrections.
- **Position**: `pi-dispatch:sched-stalls` is a HASH keyed by `schedulerId` alone, with no host and no
  queue in the field name. Two hosts running the same cron trigger therefore share one counter and one
  `hdel`. This was unreachable while there was one worker; it is reachable now, and it is reachable in both
  directions. Two hosts each stalling twice trip a threshold of four that neither one crossed, and one
  host's successful run clears a count the other host earned.
- **Why it was not fixed here**: the field name is the counter's identity, so changing it resets every live
  count in every running deployment on upgrade. That is a defensible thing to do deliberately and a bad
  thing to do as a drive-by inside a slice about something else.
- **What bounds it meanwhile**: the stall guard is a MONEY backstop against a scheduler that fires and
  never completes, and both error directions are bounded and loud. Tripping early tears down a scheduler
  and logs it; clearing early costs at most one more stall before the next trip. Neither spends.
- **What actually closed it, and TWO CORRECTIONS to the row above.** Both of this row's supporting claims
  were wrong. *"The panel already sums a HASH"* is false: `dashboard.ts` did a direct `[t.id]` index, never a
  sum. *"Changing it resets every live count in every running deployment"* is false in the strongest way --
  there have never been any live counts, because `start.mjs` called `guard.onStalled(...)` while
  `makeStallGuard` returns the listener itself, so every stall threw a `TypeError` and the money backstop
  never counted one from the day it was wired.
  The namespacing this row proposed is also still refused, on the reasoning recorded when it was declined as
  a standalone change: the counter's subject is the SCHEDULER, and in the reachable multi-host case both
  workers drain one queue and BullMQ dedups to one scheduler, so pooling is correct. Namespacing by host
  would require N stalls on ONE host, doubling the paid re-runs before the backstop fires.
  What was really wrong was the WINDOW, not the field: one `EXPIRE` on the whole hash meant any scheduler's
  stall pushed every other scheduler's count forward. Each scheduler now counts under its own key with its
  own expiry, which is per-entity by construction rather than by bookkeeping.
...
- **AMENDED (issue #337, 2026-09-21; widened again at #367, 2026-09-22)**: the belt is no longer
  `failedReason`-only, and it is no longer RUN_DETAIL-only either. EVERY pane that prints a record applies
  the same class through one helper -- `cellOf` in the framed panel (LIST, RUN_DETAIL, HELD, HELD_LIST and
  its armed cancel question, FAILED) and `cell` in `render.mjs` for the plain-text twins a non-TTY panel
  and the unframed degrade use, which is the path a belt living inside the frame builders misses entirely.
  It started at RUN_DETAIL because that pane renders `reason`, `host`, `backend`,
...
- **Why the panel cannot just be told, and the answer SPLITS by key.** Nothing in this project loads a
  `.env` into a process (`docs/secrets.md`) and the panel may be started from anywhere, so the only thing
  that moves settings to it today is the deployment pointer.
  - **`PI_EGRESS`, `PI_EGRESS_PROXY` and `DOCKER_HOST` are pointer-INELIGIBLE by policy.**
    `INT-DEPLOYMENT-POINTER-CONTRACT` and `POINTER_ENV_ALLOWLIST`'s own header state it: the file carries
    paths and URLs and never capability grants, and a pointer that could set `PI_EGRESS=0` or redirect
    `DOCKER_HOST` would be exactly the second unreviewed door that allowlist exists to forbid. For these
    three, saying what was read is the answer and widening the pointer is not.
  - **`PI_SANDBOX_DIR` is not in that category and this row should not borrow the argument for it.** It
...
- **Two pre-start refusals exit 1 and are therefore SILENT, measured against docker 27.4.0** (issue #367,
  ...
  **Widening to 1 is not the answer** and is not what is deferred here: 1 is the commonest shell exit there
  is, and the row above already records why no exit code separates the runtime from the interactive bash.
  What would close it is turning both into PRE-LAUNCH refusals rather than post-hoc exit-code readings:
  `sandbox-cli.mjs` already refuses a missing TTY before anything can half-succeed, and the panel resolves
  the docker endpoint elsewhere. That is a change to what the panel refuses and when, which is why it is
  recorded rather than folded into a control-byte fix.
- **A healthy shell whose last command was a typo buys a 2.5 s pause** (issue #367, item 3). The sandbox
  runs `--entrypoint bash -i`, so `docker run` returns bash's status and bash returns 127 for `command not
  found`. The message is worded so it is true either way ("if no shell opened, ..."), but `pauseForMessage`
  is a fixed timer, so a typo at the end of a debugging session costs one pause on the way back to the
  panel. That is the stated trade for not letting a genuine `exec bash failed` go silent again, and
  separating the two needs the same docker read the row above describes.
...
| 2026-08-30 | Issue #57, corrections after an adversarial pass. **NEW `OQ-033`**: `pi-dispatch:sched-stalls` is keyed by `schedulerId` alone with no host or queue namespace, so two hosts running one cron trigger share a counter and an `hdel` -- they can trip a threshold neither crossed, and one host's success can clear a count the other earned. Unreachable while there was one worker, reachable now. NOT fixed in this slice deliberately: the field name is the counter's identity, so changing it resets every live count on upgrade, which is a defensible deliberate act and a bad drive-by. Bounded meanwhile because both error directions are loud and neither spends. **`OQ-008` UNCHANGED, checked**: the kill switch now reads BullMQ's own meta keyspace alongside the registry, which moves a decision OFF Redis-held state that can go stale and back onto the queues themselves, so it strengthens rather than tests this row -- deleting the whole `host:*` keyspace once again changes nothing any surface decides. **`OQ-031` UNCHANGED, checked**: folder identity is untouched. **`OQ-032` UNCHANGED, checked**: forge routing is still the receiver's, and nothing here moved it. **Code evidence**: worker/src/queue.mjs -> discoverHostQueues, unionQueueNames, fleetQueueNames; worker/src/cron.mjs -> reconcileGated; worker/src/schedules.mjs -> authoredCron; admin/src/read-model.mjs -> readFleetQueues; admin/src/dashboard.ts -> createDashboardDeps. |
...
| 2026-09-21 | Issue #337, items 2, 3 and 4. **NEW `OQ-038`** (WATCH): the panel resolves every sandbox setting from the environment pi was started in and cannot see the deployment's. The egress posture is now SAID on the line, which is what the issue asked for, and this row records the rest rather than letting one fixed instance imply the whole thing is answered: the retention window it reports, the idle timeout the shell gets, which directory of retained runs it can see, and `DOCKER_HOST`, which is the sharpest and did NOT get a line -- a Podman deployment points it at Podman's socket, so a panel reading it from its own process opens the sandbox on a DIFFERENT DAEMON than the worker's jobs ran on, and that fails as a missing image rather than visibly. Widening the deployment pointer is not the answer for the capability-shaped keys (`INT-DEPLOYMENT-POINTER-CONTRACT` and `POINTER_ENV_ALLOWLIST`: paths and URLs, never capability grants), while `PI_SANDBOX_DIR` is an ordinary path the allowlist could carry and the row says so rather than borrowing the capability argument for it, so the resolution named is a deployment-owned answer the panel can read without a grant. The launcher being hard-wired to the docker CLI belongs to `#354` and is named as out of scope rather than omitted. **`OQ-035` AMENDED**: its belt is no longer `failedReason`-only, and the reason it gave for leaving the record's own fields alone (a fixed enum written by the worker) is still true and is still why the wider scrub is belt-and-braces rather than a boundary. **`OQ-016` UNCHANGED, checked**: the suspend-and-hand-over pair is untouched; what changed is that a failed hand-over now stops before the redraw. **`OQ-025` UNCHANGED, checked**, and NOT cited for the pointer's policy, which it does not state: the doctrine quoted is `INT-DEPLOYMENT-POINTER-CONTRACT`'s and `deployment-pointer.mjs`'s own. **`INT-DEPLOYMENT-POINTER-CONTRACT` UNCHANGED, checked, and cited**: its allowlist is why three of the keys cannot reach the panel that way, and why the fourth could. |
| 2026-09-21 | Issue #336. **`OQ-037` UNCHANGED, checked**: rootless Podman is still refused, and nothing here changes what would close it. `INT-SESSION-STORE-CONTRACT` now states which transcript OWNERSHIP the store's lock, its staleness takeover and its reaper assume, and names the one file the worker does not own -- the per-job `/session/current.jsonl`, written by the job user inside the container and only ever READ host-side -- together with the fact that under rootless Podman without `--userns=keep-id` that assumption may not hold, that this has NOT been measured, and that the symptom would be a bounded, named promotion refusal rather than a corrupt store. **`OQ-031` UNCHANGED, checked, and now more reachable**: the takeover compares a filesystem mtime against a process clock, which is the second such comparison in the tree after `triggers-file.mjs`, and a shared `PI_SESSIONS_DIR` is exactly the two-hosts-one-tree shape that entry records. The skew analysis is carried across from the first rather than re-derived, both signs and all. **`OQ-014` UNCHANGED, checked**: nothing about who may be handed a transcript moves. |
