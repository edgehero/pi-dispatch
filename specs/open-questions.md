...
- **Status**: **OPEN** — found while reviewing the placement slice.
...
- **What would close it**: `<schedulerId>@<host>` as the field, plus a one-time migration that reads the
  old field and drops it. The panel already sums a HASH, so the display cost is a group-by.
- **Raised by**: issue #57.
