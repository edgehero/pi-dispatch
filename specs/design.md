...
- **Why**:
  ...
  - **The injection boundary holds by placement, not by filtering.** Raw container `.log` output is
    ...
    plain-text twins for a non-TTY panel and the unframed degrade -- goes through one `cellOf`, applied
    inside the value before styling (the styler emits its own escapes, OSC-8 hyperlinks included, so scrubbing a composed
