...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **What the estimate cannot see, the page measures** (issue #422): scripts before U+1100 whose glyphs
  ...
  bidi algorithm, and a mark after them put the ellipsis inside the number, so they decide nothing. A
  ...
  otherwise change only by the script. **Residuals, stated**: with the script off every #418 residual stands; the cron cut is
