...
- **What cleans up, and why never `-f`** (issues #357, #350). The job path removes its own network in a
  ...
  - **The fourth namespace: a SESSION network, swept on the retained directory's clock** (issue #337).
    ...
    price of the first half is that a network outlives its directory by one whole pass. A third listing covers the LAUNCH
    WINDOW, `docker ps -a --filter status=created`, read before anything else and failing the sweep closed
    when it does not answer. It exists because of a measurement that CORRECTS one this project shipped under
    #357: `.Containers` lists running endpoints only, and the earlier comment generalised that to "listed and
