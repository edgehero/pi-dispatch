...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **What the estimate cannot see, the page measures** (issue #422): scripts before U+1100 whose glyphs
  ...
  script in the one script element) measures every left-anchored label with the browser's own font and,
  where one runs past its box, shortens it on grapheme boundaries with a measured ellipsis. The box is
  read from the markup, not from an attribute emitted for it: a sibling rect whose band holds the
  baseline and which holds the text's start (its right edge bounds the text) or starts after it (its left
  edge does), and the next left-anchored label on the same baseline; the nearest bound wins, 6px of
  padding is kept, and a label no bound reaches (a value drawn after its bar) is left alone.
  **Geometry never moves**: the static estimate still sizes every chip and every wire, the page only ever
  shortens a label, and a page with its script off is exactly the static page, which is why a chip is
  never grown to fit instead (the layout is computed ahead of time and the wires would have to follow
  it). A label cut this way gains a title with its full text unless its group shows a tooltip of its own,
  the full text is kept on the element so a second pass after the fonts arrive starts from it, and a box
  too small for a single character keeps the ellipsis alone. An emoji's ink runs a few pixels past the
  advance width the page measures; the padding absorbs it. The cron re-arm label, centred under its chip
  with no box, is the one label the builder cuts instead: to the chip's width at 6px a column of its
  10px font, so a common pattern is drawn whole. A loop hint is trimmed by whole clusters, so a Prepend
  keeps the space it joined. ASCII scenes are byte-identical; the page's bytes change only by the script.
  Measured in headless Chrome by `.github/scripts/label-fit-check.mjs` (a local gate, not CI) over a
  corpus of every script named here: 21 labels ran past their box with the fit stripped and none with
  it, and no label of a plain ASCII page was changed. The
...
| 2026-09-25 | Issue #422. **`DES-ADMIN-VIA-PI-EXTENSION` AMENDED**: the residuals #418 stated (labels the column estimate cannot size) are closed by MEASURING at view time rather than by widening the estimate. **Why not a wider table**: the page's font is the viewer's system stack (`-apple-system`, `Segoe UI`, Roboto and on), so a table measured in one browser on one OS is another's guess, and a table that covered wide Latin letters would move every ASCII chip; a measurement in the page is exact wherever it runs. **Why not a chip that grows**: the layout, the column pitch and every wire are computed ahead of time from the chip widths, so a chip grown in the browser would need the wires rerouted there too; the page only ever shortens a label, never moves a shape. **What changed**: `graph-html.mjs` exports `FIT_JS` (the insights page runs it ahead of `PAGE_JS`), which measures each left-anchored label against the box the markup already gives it and cuts on grapheme boundaries with a measured ellipsis, adding a title of the full text where the group shows no tooltip; the cron re-arm label is cut in the builder to its chip's width; `findLoopHints` trims whole clusters through the new `panel.mjs` `trimClusters`, so a Prepend keeps the space it joined. **Unchanged**: the static estimate (`labelColumns`, its sweep and every chip width), so ASCII scenes are byte-identical and a page with its script off is the page it was. **Measured** by the new local gate `.github/scripts/label-fit-check.mjs` in headless Chrome: 21 of 116 labels ran past their box with the fit stripped, none with it, and no label on a plain ASCII page changed. `REQ-INSIGHTS-HTML-EXPORT` UNCHANGED, checked (the artifact binds no port and loads nothing; the fit reads only the page's own DOM); `DES-GRAPH-EDGE-DERIVATION` UNCHANGED, checked (no edge or label vocabulary moves). Code evidence: `graph-html.mjs` `FIT_JS` (`fitLabels`), `routeWire` (`labelMax`), `wireSvg`; `insights-html.mjs` script assembly; `graph-model.mjs` `findLoopHints`; `panel.mjs` `trimClusters`. |
