...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **theme-colored** panel (color via pi's injected `Theme`, applied post-layout; CORRECTED under issue #382,
  ...
  combining content too, not only ASCII, with an emoji ZWJ sequence the one shape it draws short) carrying a
...
- **Why**:
  ...
  - **The injection boundary holds by placement, not by filtering.** Raw container `.log` output is
    ...
    **It is held there by a sweep of every code point there is**, plus every character-with-U+FE0F pair,
    asserting that the table never measures NARROWER than the renderer and that every place it measures
    wider is an unassigned code point. **The direction is the point, and "over-counting is harmless" would be
    ...
    safer of two bad shapes rather than a harmless one. **Its one remaining limit**: the table sums
    CODE POINTS, so an emoji ZWJ sequence counts every member and comes out wider than the single glyph a
    terminal draws, 6 against 2, pinned as a disagreement rather than left to surprise someone.
