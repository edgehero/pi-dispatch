...
- **Why**:
  ...
  - **The injection boundary holds by placement, not by filtering.** Raw container `.log` output is
    ...
    reads as the boundary invites the placement rule to be relaxed later. Every record string RUN_DETAIL
    prints goes through the same C0-plus-DEL class the `failedReason` belt uses, applied inside the value
    before styling (the styler emits its own escapes, OSC-8 hyperlinks included, so scrubbing a composed
    line would destroy the link and the pane's width math). The record is PII-free and worker-written, so
...
- **What cleans up, and why never `-f`** (issues #357, #350). The job path removes its own network in a
  ...
  - **The namespace is the NAME, not the filter.** `docker`'s `--filter name=` is a SUBSTRING match, so
    ...
    back. The sweep now decides on the anchored shape the producer builds
    (`networkNameFor(jobContainerName(id))`), derived from both constants rather than spelled again, and the
    container half takes the same narrowing. This matters MORE after this change than before it: the old
    sweep would only have failed to `rm` a foreign network that was in use, while this one detaches first,
    so without the anchor it would strip an operator's containers off their own network.
...
- **REGISTRATION IS IN-TREE, and that is a decision rather than the gap issue #342 filed it as.** `startWorker`
  ...
  **It costs an adapter author nothing they were not already paying**, which is why this does not contradict the
  ...
  here, so it needs no export and nothing in this refusal blocks it. What would reopen the question is an
  operator installing this package from npm who wants to register an adapter with no checkout at all, which
  is the case an export would serve and which nobody has asked for.
...
- **`credentialTransit` is now `enforced`, and only while OBSERVED (issue #278).** The worker asks the docker
  ...
  **The floor reads the observed word, not the declared one** (`unobservedFloor`, `observationRefusals`),
  ...
  with the fixed comment instead of retrying. **What is displayed** is the endpoint verbatim when it holds no
  `@`, else only `scheme://host` from a URL that parses without a password, else nothing of it: every rule
  that edited the string kept part of some password.
...
| 2026-09-21 | Issue #342 and issue #357 item 3, the backends page. **`DES-CONTAINER-BACKEND-REGISTRY` AMENDED**, one bullet, and it is new ground against this entry's own framing rather than a revision row beside it: the entry says the table exists so a second venue can be added "without reading the worker's source" and that an adapter is "written elsewhere by someone who will never run `npm test` here", so declaring registration IN-TREE ONLY had to be argued inside the entry or it would have read as its opposite. The argument is that it costs an adapter author nothing they were not already paying: `BACKENDS_TABLE`'s declaration is in-tree BY NECESSITY, on this entry's own ground that a venue nobody can read a declaration for is a venue nobody can reason about, so an adapter already lands one file here and its code can still live anywhere. Exporting `startWorker` was the alternative and is refused because its second argument is the worker's whole dependency-injection bag, test seams included, and an export makes every seam a public API a release has to keep. #354 is named as what reopens it: an out-of-tree `podman` backend needs exactly the entry point this declines, and #342 asked to be decided alongside the Podman route rather than after it. **`docs/backends.md` gains the job-user TIMING paragraph and its first test of any kind.** The page's refusal sentence named seven of the code's eight causes in six clauses, one of which covers two, and omitted `runtime-unreadable` entirely, which is what an untested page does; it now names both sets, boot-refusing and per-job, and both are BOLTED to `jobUserBootRefusal(decision, defaultBackend)` rather than to `BOOT_REFUSING_JOB_USER_CAUSES`, because the page's claim is not that four names sit in a Set, it is that they stop a boot AND only while `local` is the default venue, and only the function carries the second half. Adding, removing or renaming a member of that Set, and dropping the venue condition, each turn the new test red. The FIRST version of that test was green for eleven of seventeen wrong pages, which an adversarial review found and which is the more useful half of this row. The lists were genuinely bolted; what was not was WHICH TEXT the extractor reads. A correct copy of both lists hidden in an HTML comment satisfied every assertion while the rendered bullets said the opposite, because markdown hides comments from a reader and `indexOf` reads raw bytes. A second marker pair pasted higher up the page made the test read a decoy. Prose after each bullet's first full stop, which the extractor stops at, was free to reassign a cause outright, and a cause named without backticks was invisible to the regex while plain to a reader. So the block is now sliced AFTER the opening marker with comments stripped, both markers must occur exactly once, every cause must be named exactly ONCE anywhere in the block, and the markers were widened to enclose the whole passage rather than the two bullets. The page's conditional half gets the weakest assertion in the file, a required phrase with the venue name derived from `DEFAULT_BACKEND`, because behaviour can be derived and a page's silence about it cannot; it is kept because the alternative is a page free to call the boot exit unconditional while its own test proves otherwise. All seven attacks are red now, and the residual is written into the test rather than hidden: a sentence naming NO cause is invisible to all of this, which is the same limit `podman-doc.test.mjs` already records. The page also states what is in NEITHER set: an unanswered daemon reads `unknown`, never cached and never a boot exit, and a job picked up while the answer is unknown is an infra RETRY rather than a refusal (`CONST-RETRY-INFRA-ONLY`, `processor.mjs` throws where a policy refusal returns). **`DES-JOB-USER-INFERRED-READ-BACK-ON-REQUEST` UNCHANGED, checked, and cited**: its residual already carries the conditional this paragraph states, corrected under #348 earlier in this round, so the page and the entry landed agreeing rather than one correcting the other. **`DES-PODMAN-THROUGH-ITS-DOCKER-API` UNCHANGED, checked**: no route, refusal or measurement moves. **`DES-WORKER-ON-HOST` UNCHANGED, checked**: the worker still runs on the host and shells out. `worker/package.json`'s exports map is byte-unchanged and `worker/test/publish.test.mjs` still passes, which is the check that would have caught an export slipping in with the prose. One measurement lands with it, the last of #357's four items: `docs/egress.md` hedged an external name resolved directly from an internal network as "about 10 seconds on the host this was first measured on". Measured on docker 27.4.0 instead, and the variable was the wrong one: the resolver INSIDE the container decides it. The job image, Debian and glibc, gives up in about 10 ms (six runs, 6 to 23, all `EAI_AGAIN`) because it takes the embedded resolver's failure as final; a musl image on the same network waits out musl's own 5 s timeout (five runs, 5013 to 5025). So the number that describes a job here is milliseconds, and a multi-second reading is a property of the client. **Code evidence**: docs/backends.md; worker/test/backends-doc.test.mjs; docs/egress.md. |
