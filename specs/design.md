...
- **What cleans up, and why never `-f`** (issues #357, #350). The job path removes its own network in a
  ...
  - **The namespace is the NAME, not the filter.** `docker`'s `--filter name=` is a SUBSTRING match, so
    ...
    **The cost is real and is accepted rather than argued away.** A compose project named `pi-job-tools`
    ...
    that same operator's `pi-job-`-prefixed containers were already `rm -f`'d at every boot under the old
    code, which is the unrecoverable half, while the network half is recoverable -- by `docker compose up -d`
    for the case that is likely to meet it, NOT by a `docker network connect`, which answers `network not
    found` because this sweep removes the network rather than only detaching from it, and not by a hand
    `network create` either, which loses the compose labels and aliases and leaves service-name DNS broken
    (measured on docker 27.4.0; an earlier draft of this entry weighed the cost as "one `docker network
    connect`" and had not run it). And a rule that destroys half of a stack while sparing the other half by
    an accident of suffix is not a protection, it is a coin flip. `SECURITY.md`'s *What is NOT defended*
    states the whole of it for operators.
