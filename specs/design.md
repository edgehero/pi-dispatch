...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **Its labels are sized and cut in estimated COLUMNS, and no cut on the page leaves half a character**
  ...
  which the one-column estimate cannot see, a few code points past it that draw far wider than two
  columns (U+FDFD, the long dashes U+2E3A and U+2E3B), and any label that is never cut (a group title). The
