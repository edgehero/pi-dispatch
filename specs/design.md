...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **Its labels are sized and cut in estimated COLUMNS, and no cut on the page leaves half a character**
  ...
  the keycap's enclosing mark and for anything from U+1100 on, and otherwise the code units a code point
  ...
  Both sinks, the markup and the embedded JSON, make every string well-formed, so half a pair that
  ...
  a keycap or a ZWJ family now fits; what still runs past its chip is stated: scripts before U+1100
  whose glyphs draw much wider than 8px (Malayalam, Myanmar, Tamil, Thai) and runs of wide Latin letters,
  which the one-column estimate cannot see, and any label that is never cut (a group title). The
