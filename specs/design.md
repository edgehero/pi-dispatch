...
- **`credentialTransit` is now `enforced`, and only while OBSERVED (issue #278).** The worker asks the docker
  ...
  **The floor reads the observed word, not the declared one** (`unobservedFloor`, `observationRefusals`),
  ...
  logs only when the answer changes. **Determinate only on an allow-list**, `transient.mjs`'s shape: no docker
  binary, and the CLI's two recognised configuration refusals (a context that does not exist, a `DOCKER_HOST`
  it cannot parse); every other failure to ask, a `permission denied` on the context store included, is
  transient. **Residuals**: the form of a unix socket or a loopback port says nothing about a tunnel behind
