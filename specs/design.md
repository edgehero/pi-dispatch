...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **Its labels are sized and cut in estimated COLUMNS, and no cut on the page leaves half a character**
  ...
  which the one-column estimate cannot see; past U+1100, glyphs a browser draws wider than two columns
  (measured: about two thousand code points, among them Ethiopic, Khmer, Javanese and Balinese signs,
  cuneiform, Egyptian hieroglyphs, the Arabic presentation ligatures with U+FDFD the widest, the long
  dashes and the per-ten-thousand sign, every one counted as one column on `main`); and any label that
  is never cut (a group title). The
