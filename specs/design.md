...
- **Decision** (issue #354): the backend table's second entry, `podman` (`remote: false`), is this worker account's
  ...
  reached with `--network=pasta:--no-map-gw` on the command line). An egress-armed job is on its own per-job bridge
  network and unaffected. Nothing observes it yet, and neither does anything observe a containers.conf
  ...
  is judged BEFORE the venue's observations, at boot and per job (`observationPreflight` passes it through for
  `jobUserPreflight` to refuse): a rootful, remote or absent Podman misses a floored observation too, and judging that
  first told each job to fix a mounts.conf or delegate controllers, and at boot stopped a worker whose default venue
  was fine. `pi-dispatch sandbox` refuses a
