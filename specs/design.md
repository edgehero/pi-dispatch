...
- **Residuals**:
  ...
  - Rootful Podman with containers.conf `userns="auto"` never reports `name=userns`, so the rule decides `worker`
    ...
    because the uid is outside the allocated namespace (125, refunded), or it starts and the runner's `/job` check
    refuses it (exit 2, slot kept).
