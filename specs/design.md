...
- **Two OPTIONAL bundle members, `observationPreflight` and `jobUserPreflight`** (issue #354). They were
  ...
  venue, `DES-PODMAN-NATIVE-ROOTLESS-BACKEND`). A venue without it behaves exactly as before.
...
- **Decision** (issue #288): the post-spend terminals comment through the EXISTING adapter -- worker-abort,
  ...
  **Issue #437 adds one row, `provider-auth-refused`**: a container exit 2 whose runner named a provider's
  401/403 refusal of the credential. Its sentence ("Stopped: the AI provider refused this worker's credentials or access (HTTP 401 or 403). The operator needs to check the provider key and what it is allowed to use. Not retried.") is fixed and path-free like its
  siblings and never carries the provider's message, which may echo a key fragment. It says "or access"
  and "what it is allowed to use" because a 403 is as often a working key refused a model or a route
  (a gateway's policy, a model the account may not sample) as a bad key.
...
- **Decision** (issue #354): the backend table's second entry, `podman` (`remote: false`), is this worker account's
  ...
  venue.) What the argv CANNOT pin is the network's options: `[network] pasta_options` and slirp4netns's
  `network_cmd_options` are appended to what the command line names, never replaced, so an account that maps host
  loopback for its own containers gives an egress-off job the host's `127.0.0.1` services (measured, Fedora 44 with
  pasta: a loopback-bound Valkey refused by default, reached with `--map-host-loopback` in containers.conf, and still
  reached with `--network=pasta:--no-map-gw` on the command line). An egress-armed job is on its own `--internal`
  per-job network, which has no route out, so the job itself cannot use the mapping (reasoned, not measured); the
  proxy, on a named bridge behind the rootless network namespace, may, which is an egress question for issue #428. Nothing observes it yet, and neither does anything observe a containers.conf
  `annotations = ["run.oci.keep_original_groups=1"]`, which `dockerExtra` refuses: both are named residuals, the
  account's own configuration as daemon.json is `local`'s, issue #428. The run, the stop, the reaper, the image and egress preflights and the job networks are
...
| 2026-09-26 | Issue #437, review round 1. **`DES-TERMINAL-COMMENTS-AND-FAILURE-HOOK` AMENDED**: the `provider-auth-refused` sentence now reads "credentials or access" and asks the operator to check "the provider key and what it is allowed to use", because a 403 is as often a working key refused a model or a route as a bad key, and the first wording blamed the key for both. `HOOK_POLICY_REASONS` is now built by spreading `RUNNER_POLICY_REASONS`, and a test holds every member of that set to a `TERMINAL_COMMENTS` row, so the two lookups cannot drift from the list they key off. **Code evidence**: worker/src/processor.mjs -> TERMINAL_COMMENTS (now exported for that test); worker/src/start.mjs -> HOOK_POLICY_REASONS. |
