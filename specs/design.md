...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **theme-colored** panel (color via pi's injected `Theme`, applied post-layout; CORRECTED under issue #382,
  ...
  draws short, and one measured cluster shape it draws long) carrying a
...
- **Why**:
  ...
  - **The injection boundary holds by placement, not by filtering.** Raw container `.log` output is
    ...
    **It is held there by two sweeps: every code point there is, and every character followed by U+FE0F and
    by a keycap**,
    asserting that on every one of those the table never measures NARROWER than the renderer, and that every
    place it measures wider is a declared departure: an unassigned code point, or a keycap asked for on a
    base that has no keycap form. **That guarantee is per code point and per those pairs, not for every
    string**, and a review pass measured where it stops: the renderer computes a cluster's base AFTER
    stripping leading non-printing characters, so a cluster BEGINNING with one of 2,616 zero-width
    characters and continuing with one of exactly four tails (U+FF9E, U+FF9F, U+0E33, U+0EB3) has that tail
    counted twice, and we count it once. It is filed as #417 rather than matched here, under the round cap. **One leader of that shape is fixed rather than filed**, because its containment was one line: a LONE SURROGATE breaks a cluster the same way, and `clip` used to return its input unchanged when it fitted, so an orphan the count had already removed was still in the text handed to the renderer. It steps the fitted line too now, which makes the rule this module states -- half a character is removed where text ENTERS -- true of the TEXT rather than only of the count. **The direction is the point, and "over-counting is harmless" would be
