...
- **What ARMING cannot do, on macOS, measured rather than reasoned** (issue #373): a watch does not observe
  an edit made in the moments right after `watch()` returns. libuv answers `fs.watch` from
  `uv__fsevents_init`, which only queues the path and signals its CoreFoundation thread; that thread then
  destroys the process's single FSEvents stream and creates a new one, for every watched path at once,
  starting from "now" (`src/unix/fsevents.c`, libuv 1.49.2, the version Node 23.5 bundles). An edit landing
  before the new stream is live is never delivered, and it is not delivered late either: it is gone. Any
  watcher armed or closed anywhere in the process reopens the same window, which is why the loss rate rises
  with concurrency: measured against the real `receiver` boot in 40-trial runs, 0 lost with one boot at a
  time on an idle machine, 24 lost with four boots in one process, and 1 lost with one boot at a time while
  two full suites ran beside it. Linux's inotify registers the watch before `fs.watch` returns, so there an
  edit can be late but not lost. Two consequences worth separating. For a TEST, the answer is to repeat the
  edit rather than to wait longer, which is what `receiver/test/start.test.mjs` does. For an OPERATOR, the
  window is real but bounded by boot: both services read the file first and arm the watch afterwards, so an
  edit made in that gap is missed until the next edit or restart. Neither service re-reads after arming,
  which is the wider version of the same gap and is tracked as issue #386 rather than closed here.
...
| 2026-09-22 | Issue #373, the receiver's triggers-watch test. **`DES-WATCHERS-CLOSE-WITH-THE-WORKER` AMENDED** with what ARMING cannot do on macOS, measured against libuv 1.49.2's `src/unix/fsevents.c` and against the real receiver boot: `fs.watch` returns before the FSEvents stream that serves it exists, so an edit made in that window is lost rather than delayed, and every watcher armed or closed in the process reopens it. The test answer is to repeat the edit, not to wait longer; the operator-facing half of the same gap (neither service re-reads after arming) is issue #386. No behaviour changed here, and no other entry moved: `DES-CRON-VIA-BULLMQ-SCHEDULER` and `DES-TRIGGERS-UNIFIED-FILE` describe reload CONTENT rather than arming, checked. | `receiver/test/start.test.mjs`, `receiver/src/start.mjs` |
