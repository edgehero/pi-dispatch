...
- **Residuals**:
  ...
  - Doctor's warning that a system unit runs the worker as another account compares only the unit's own explicit
    ...
    job one at a time, so a fleet whose default venue is elsewhere runs on with a root worker and says so per job
    rather than at boot. Where the unit DOES name uid 0 explicitly, doctor states the refusal instead of sending the
    operator to find it, and it asks `decideJobUser` for the answer rather than assuming one: `worker-is-root` is
    that function's LAST row, so a Mac, an endpoint that is not on this host, a rootless or userns-remapped daemon
    and Docker Desktop on Linux all answer for uid 0 first, and two of those are not refusals at all. The fix text
    is whichever cause this host's own facts give uid 0, and the re-run instruction stays where there is none (#348).
