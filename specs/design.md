...
- **`credentialTransit` is now `enforced`, and only while OBSERVED (issue #278).** The worker asks the docker
  ...
  **The floor reads the observed word, not the declared one** (`unobservedFloor`, `observationRefusals`),
  ...
  `gh auth token` precedent -- every exit measured is a configuration refusal (a missing context, an
  unparseable `DOCKER_HOST` or context file), and splitting out the one that can pass (`permission denied` on
  the context store) would take a table of docker's prose. That is a named residual: under a floor it exits 2.
