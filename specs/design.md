...
- **What ARMING cannot do, on macOS: a measured consequence, with the mechanism read from libuv's source**
  ...
  full suites ran beside it. Churn is what matters, not neighbours: an already-live watcher missed nothing
  in 80 writes while ONE unrelated watcher was armed and closed beside it, and missed 27 of 60 while twenty
  were. **The 0 row proves less than it looks**: a write made just BEFORE arming is itself usually delivered
  after arming, so at idle a reload arrives whether or not a later edit does. Linux's inotify registers the
  watch before `fs.watch` returns, so an edit cannot be lost to this window there, though a full kernel
  queue drops events on any platform. Two consequences worth separating. For a TEST, the answer is to repeat
...
| 2026-09-22 | Issue #373, the receiver's triggers-watch test. **`DES-WATCHERS-CLOSE-WITH-THE-WORKER` AMENDED** with what ARMING cannot do on macOS, measured against libuv 1.49.2's `src/unix/fsevents.c` and against the real receiver boot: `fs.watch` returns before the FSEvents stream that serves it exists, so an edit made in that window is lost rather than delayed, and every watcher armed or closed in the process reopens it. The test answer is to repeat the edit, not to wait longer; the operator-facing half of the same gap (neither service re-reads after arming) is issue #386. No behaviour changed here, and no other entry moved: `DES-CRON-VIA-BULLMQ-SCHEDULER` and `DES-TRIGGERS-UNIFIED-FILE` describe reload CONTENT rather than arming, checked. **Code evidence**: `receiver/test/start.test.mjs`'s arrival poll and its re-write, `receiver/src/start.mjs:299-326`. |
