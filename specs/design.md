...
- **REGISTRATION IS IN-TREE, and that is a decision rather than the gap issue #342 filed it as.** `startWorker`
  ...
  **It costs an adapter author nothing they were not already paying**, which is why this does not contradict the
  ...
  What would reopen this is a venue whose adapter cannot live in this repository at all. `podman` is the live
  example and the reason #342 asked to be decided alongside the Podman route: in-tree it needs no export, and
  out-of-tree it needs exactly the published entry point this refuses, so issue #354 is where that lands.
