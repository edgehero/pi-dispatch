...
- **Residuals**:
  ...
  - Doctor's warning that a system unit runs the worker as another account compares only the unit's own explicit
    ...
    rather than at boot. Where the unit DOES name uid 0 explicitly, doctor says it outright and gives
    `worker-is-root`'s own fix text rather than telling the operator to re-run as root and read it there (#348).
