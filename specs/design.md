...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  **theme-colored** panel (color via pi's injected `Theme`, applied post-layout; CORRECTED under issue #382,
  ...
  colour is post-layout by construction now rather than by convention. Even so, pi's `visibleWidth` frames
  it only for ASCII: this module counts UTF-16 units, so a CJK or combining character still frames ragged,
  which is issue #401) carrying a status header, day/week/month **SPEND meters** (colored by the
