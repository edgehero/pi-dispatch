...
- **Who authors which comment** (issue #288, closing the acceptance's paid half): the AGENT authors the
  ...
  401/403 refusal of the credential, and its sentence tells the requester the operator must check the
  provider key), and the final infrastructure failure. Once-ness for the infra class is BullMQ's own terminal decision
...
- **`doctor` reads the deployment's `.env` for two named keys, to decide what to say AND whether to fail**
  ...
  **WHAT ONE LINE DOES TO ANOTHER IS PER LOADER**, measured on systemd 252 against the four shells rather
  ...
  working directory. It still configures nothing, writes nothing and runs nothing. **A `PI_ENV_SETUP` script THAT EXISTS
  ...
  `--env-setup` deployment, and without the existence condition doctor softens a refusal on the strength of
  a script it has just reported missing. A whitespace-only value is configured, not unset, because the
