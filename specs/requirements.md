...
- **Statement**: An operator shall be able to stop ONE job from a surface that needs no model: the CLI
  (`pi-dispatch cancel <jobId>`, VALKEY_URL-only like `pause`) and the panel (`x` on the ACTIVE row; the
  held drill-in behind `h`). An active job is aborted on whichever host owns it and its run record says
  `outcome: "policy", reason: "operator-cancel"`; a queued, delayed or held job is removed and records
  NOTHING, because it never ran. A cancel that no reachable worker acknowledges is a named refusal,
...
- **Acceptance**: Given `PI_EGRESS=0`, a job's docker argv and container env are byte-identical to ones
  ...
  rather than by the agent; and the network is removed when the container exits, or, where a worker died
...
- **Fail OPEN, and say so.** A missing, expired, oversized, unparseable, locked or foreign transcript, one
  written in another venue, one whose key directory in the store is not a directory at all, a
...
- **Acceptance**: Given a trigger carrying `run.secrets`, it loads in all three loaders (worker, receiver,
  ...
  `secret-name-reserved` when a key collides with **any variable pi reads the resolved
...
- **Statement**: The CLI shall take a fresh machine to a preflighted deployment through **create-only
  ...
  predicate would have refused every key on `C:\Program Files\...`; what it cannot take is `%`, a double
  quote, or a line break. Windows paths are written with forward slashes, which Node accepts and which
...
- **`doctor` reads the deployment's `.env` for two named keys, to decide what to SAY** (issue #357). The
  ...
  `$`, `\`, a backtick or `"`, and a bare value from a conservative unquoted set, none of them carrying a
  ...
  or CJK path is ordinary and stays plain. TWO FLAGS, not one: what a LINE assigns is a different question
  from whether the loader ends up with it, and one line elsewhere that runs, leaves a quote open or ends in
...
- **`doctor` never reports green on a credential the worker cannot spend.** The clause above makes a
  ...
  `fixAction` -- `doctor` cannot know which provider an operator meant, and never mints a credential.
- **Every environment variable a loader reads is discoverable, or is marked as not being a key.** The
  scaffold is the operator's whole view of what this system can be told: `init` copies `.env.example`
