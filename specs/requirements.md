...
- **Acceptance**: Given `PI_EGRESS=0`, a job's docker argv and container env are byte-identical to ones
  ...
  rather than by the agent, as is one reaching a listed name that resolves to this host's own loopback or
  link-local addresses (issue #428); and the network is removed when the container exits, or, where a worker died
