...
- **Statement**: The CLI shall take a fresh machine to a preflighted deployment through **create-only
  ...
  `set` keeps surrounding quotes as part of the value, so on Windows these paths are written with forward
  slashes, which keeps them inside the bare set, and a value that would still need quoting there is
  refused. So is one carrying a single quote on any platform: the shells want `'\''` and systemd does not
