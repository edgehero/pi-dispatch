...
- **Statement**: The CLI shall take a fresh machine to a preflighted deployment through **create-only
  ...
  resolves anyway and is written without comment. **A value is rendered so that both consumers read it
  back**, single-quoted when it needs it, because bare is split at a space by the shell, truncated at a
  `#`, and a double-quoted `$` is expanded, while systemd reads single quotes too; one that cannot be
  represented for both is refused rather than mangled. **`doctor --live` adds a mutation that is shown rather than consented**
