...
- **Scope**: Deployment setup, repair, and process supervision on the operator's own host —
  ...
  **`--env-setup <absolute path>`**, the one seam by which anything runs before the worker does: the
  ...
  environment. (3) With no `--env-setup`, every rendered artifact is **byte-identical** to what this
  command produced before the seam existed, on all three platforms. Not job execution, not
