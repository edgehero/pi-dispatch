...
| `not-a-regular-file` | the transcript's own name is not a regular file. The check is an `lstat`, so a symlink is never followed. Planted in the store, this job runs cold; planted by the agent in `/session`, the promotion is refused instead, so the store keeps whatever it already had |
