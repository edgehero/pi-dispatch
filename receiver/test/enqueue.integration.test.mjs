...
test("a post-queue identity refusal EXITS, code intact: the queue's live connection is released (issue #318)", { skip }, async () => {
	...
	try {
		...
		// The 15s reaper was re-measured under issue #373's survey and stays: this exit took 0.2-0.3s idle and
		// 2.2s at worst with two suites and test-count-check running beside it, and 6x that still fits. It
